...
struct CompareJetPtMuons {
    bool operator() (const VHbbEvent::MuonInfo & j1,
                     const VHbbEvent::MuonInfo & j2) const {
        return j1.p4.Pt() > j2.p4.Pt();
    }
};
struct CompareJetPtElectrons {
    bool operator() (const VHbbEvent::ElectronInfo & j1,
                     const VHbbEvent::ElectronInfo & j2) const {
        return j1.p4.Pt() > j2.p4.Pt();
    }
};
struct CompareJetPtTaus {
    bool operator() (const VHbbEvent::TauInfo & j1,
                     const VHbbEvent::TauInfo & j2) const {
        return j1.p4.Pt() > j2.p4.Pt();
    }
};
...
void HbbAnalyzerNew::produce(edm::Event & iEvent,
    ...
    const edm::TriggerNames & triggerNames_ =
        iEvent.triggerNames(*hltresults);

    int ntrigs = hltresults->size();
    if (ntrigs == 0) {
        std::
            cerr <<
            "%HLTInfo -- No trigger name given in TriggerResults of the input "
            << std::endl;
    }
    ...
    auxInfo->pvInfo.firstPVInPT2 =
        TVector3(RecVtxFirst.x(), RecVtxFirst.y(), RecVtxFirst.z());
    auxInfo->pvInfo.firstPVInProb =
        TVector3(RecVtx.x(), RecVtx.y(), RecVtx.z());
    ...
    iEvent.getByLabel(edm::InputTag("kt6PFJetsForIsolation", "rho"),
                      rho25HandleIso);
    auxInfo->puInfo.rho25Iso = *rho25HandleIso;

    edm::Handle < double >rhoNeutralHandle;
    iEvent.getByLabel(edm::InputTag("kt6PFJetsCentralNeutral", "rho"),
                      rhoNeutralHandle);
    ...
    if (runOnMC_) {
        ...
        if (puHandle.isValid()) {
            ...
            for (std::vector < PileupSummaryInfo >::const_iterator it =
                 pu.begin(); it != pu.end(); ++it) {
                int bx = (*it).getBunchCrossing();
                if (bx == 0) {
                    auxInfo->puInfo.truePU = (*it).getTrueNumInteractions();
                }
    ...
    if (runOnMC_) {
        ...
        for (size_t i = 0; i < genParticles->size(); ++i) {
            ...
            if (id == 25) {
                ...
                if (p.mother(0) != 0)
                    htemp.momid = p.mother(0)->pdgId();
                if (p.mother(0) != 0 && p.mother(0)->mother(0) != 0)
                    htemp.gmomid = p.mother(0)->mother(0)->pdgId();
            ...
            if (abs(id) == 24) {
                ...
                if (p.mother(0) != 0)
                    wtemp.momid = p.mother(0)->pdgId();
                if (p.mother(0) != 0 && p.mother(0)->mother(0) != 0)
                    wtemp.gmomid = p.mother(0)->mother(0)->pdgId();
            ...
            if (abs(id) == 15) {
                ...
                if (p.mother(0) != 0)
                    tautemp.momid = p.mother(0)->pdgId();
                if (p.mother(0) != 0 && p.mother(0)->mother(0) != 0)
                    tautemp.gmomid = p.mother(0)->mother(0)->pdgId();
            ...
            if (abs(id) == 23) {
                ...
                if (p.mother(0) != 0)
                    ztemp.momid = p.mother(0)->pdgId();
                if (p.mother(0) != 0 && p.mother(0)->mother(0) != 0)
                    ztemp.gmomid = p.mother(0)->mother(0)->pdgId();
            ...
            if (id == 5) {
                ...
                if (p.mother(0) != 0)
                    btemp.momid = p.mother(0)->pdgId();
                if (p.mother(0) != 0 && p.mother(0)->mother(0) != 0)
                    btemp.gmomid = p.mother(0)->mother(0)->pdgId();
            ...
            if (id == -5) {
                ...
                if (p.mother(0) != 0)
                    bbtemp.momid = p.mother(0)->pdgId();
                if (p.mother(0) != 0 && p.mother(0)->mother(0) != 0)
                    bbtemp.gmomid = p.mother(0)->mother(0)->pdgId();
            ...
            if (abs(id) == 4) {
                ...
                if (p.mother(0) != 0)
                    ctemp.momid = p.mother(0)->pdgId();
                if (p.mother(0) != 0 && p.mother(0)->mother(0) != 0)
                    ctemp.gmomid = p.mother(0)->mother(0)->pdgId();
...
/// photon used in isolation
    ...
    iSetup.get < JetTagComputerRecord > ().get("combinedSecondaryVertex",
                                               handle);
    computer =
        dynamic_cast < const GenericMVAJetTagComputer *>(handle.product());
    ...
    for (edm::View < pat::Jet >::const_iterator jet_iter =
         simplejets1.begin(); jet_iter != simplejets1.end(); ++jet_iter) {
        //     if(jet_iter->pt()>50)
        //       njetscounter++;
        ...
        if (runOnMC_) {
            ...
            TLorentzVector gJp4;
            if (gJ) {
                gJp4.SetPtEtaPhiE(gJ->pt(), gJ->eta(), gJ->phi(),
                                  gJ->energy());
                sj.bestMCp4 = gJp4;
                if (verbose_) {
                    std::clog << "genJet matched Pt = " << gJp4.
                        Pt() << std::endl;
                    std::clog << "genJet matched eta = " << gJp4.
                        Eta() << std::endl;
                    std::clog << "genJet matched deltaR = " << gJp4.DeltaR(sj.
                                                                           p4)
                        << std::endl;
                    std::clog << "genJet matched mother id = " << sj.
                        bestMCmomid << std::endl;
                }
            }
    ...
    for (edm::View < pat::Jet >::const_iterator jet_iter =
         simplejets3.begin(); jet_iter != simplejets3.end(); ++jet_iter) {
        //     if(jet_iter->pt()>50)
        //       njetscounter++;
        VHbbEvent::SimpleJet sj;
    ...
    for (edm::View < pat::Jet >::const_iterator jet_iter =
         simplejets4.begin(); jet_iter != simplejets4.end(); ++jet_iter) {
        //     if(jet_iter->pt()>50)
        //       njetscounter++;
    ...
    for (edm::View < pat::Jet >::const_iterator jet_iter =
         simplejets2.begin(); jet_iter != simplejets2.end(); ++jet_iter) {
        ...
        if (PileupJetIdentifier::
            passJetId(idflag, PileupJetIdentifier::kLoose)) {
            //cout << " pass loose wp";
            sj.puJetIdL = 1;
        }
        if (PileupJetIdentifier::
            passJetId(idflag, PileupJetIdentifier::kMedium)) {
            //    cout << " pass medium wp";
            sj.puJetIdM = 1;
        }
        if (PileupJetIdentifier::
            passJetId(idflag, PileupJetIdentifier::kTight)) {
            //    cout << " pass tight wp";
            sj.puJetIdT = 1;
        }
        //    cout << endl;
        ...
        for (edm::View < reco::Candidate >::const_iterator mu =
             muonsNoCuts.begin();
            ...
            if (Smpt > lep_ptCutForBjets_ && SmJdR < 0.5) {
                ...
                sj.SoftLeptdR = SmJdR;
                sj.SoftLeptPt = Smpt;
                TVector3 mvec(m.p4().Vect().X(), m.p4().Vect().Y(),
                              m.p4().Vect().Z());
                sj.SoftLeptptRel = sj.p4.Perp(mvec);
                sj.SoftLeptRelCombIso =
                    (m.trackIso() + m.ecalIso() + m.hcalIso()) / Smpt;
        ...
        for (edm::View < reco::Candidate >::const_iterator ele =
             elesNoCuts.begin();
             ele != elesNoCuts.end() && sj.isSemiLept != 1; ++ele) {

            const pat::Electron & e =
                static_cast < const pat::Electron & >(*ele);
            float Smpt = e.pt();
            float Smeta = e.eta();
            float Smphi = e.phi();
            ...
            if (Smpt > lep_ptCutForBjets_ && SmJdR < 0.5) {
                ...
                sj.SoftLeptdR = SmJdR;
                sj.SoftLeptPt = Smpt;
                TVector3 mvec(e.p4().Vect().X(), e.p4().Vect().Y(),
                              e.p4().Vect().Z());
                sj.SoftLeptptRel = sj.p4.Perp(mvec);
                sj.SoftLeptRelCombIso =
                    (e.trackIso() + e.ecalIso() + e.hcalIso()) / Smpt;
                ...
                if ((fabs(Smeta) < 2.5
                     && !(abs(Smeta) > 1.4442 && abs(Smeta) < 1.566))
                    &&
                    ((abs(Smeta) > 1.566 && (e.sigmaIetaIeta() < 0.01)
                      && (e.deltaPhiSuperClusterTrackAtVtx() < 0.8
                          && e.deltaPhiSuperClusterTrackAtVtx() > -0.8)
                      && (e.deltaEtaSuperClusterTrackAtVtx() < 0.007
                          && e.deltaEtaSuperClusterTrackAtVtx() > -0.007))
                     || (abs(Smeta) < 1.4442 && (e.sigmaIetaIeta() < 0.03)
                         && (e.deltaPhiSuperClusterTrackAtVtx() < 0.7
                             && e.deltaPhiSuperClusterTrackAtVtx() > -0.7)
                         && (e.deltaEtaSuperClusterTrackAtVtx() < 0.01
                             && e.deltaEtaSuperClusterTrackAtVtx() > -0.01)))
                    )
                    sj.SoftLeptId95 = 1;
            }
    ...
    for (edm::View < pat::Jet >::const_iterator jet_iter = jets.begin();
        ...
        if (printJet) {
            std::cout << "Jet Pt: " << jet_iter->
                pt() << " E,M: " << jet_iter->p4().E() << " " << jet_iter->
                p4().M() << "\n";
        }
        ...
        for (unsigned int iJC(0); iJC < constituents.size(); ++iJC) {
            ...
            if (printJet) {
                std::cout << "subJet Pt: " << icandJet->
                    pt() << " subJet E,M,eta,phi: " << icandJet->p4().
                    E() << "," << icandJet->p4().M() << "," << icandJet->
                    eta() << "," << icandJet->phi() << "\n";
            }
    ...
    for (edm::View < pat::Jet >::const_iterator subjet_iter = subjets.begin();
         subjet_iter != subjets.end(); ++subjet_iter) {

        if (printJet) {
            std::cout << "SubJetTagged Pt: " << subjet_iter->
                pt() << " E,M,eta,phi,Btag: " << subjet_iter->p4().E()
                << "," << subjet_iter->p4().M() << "," << subjet_iter->
                eta() << "," << subjet_iter->phi()
                << "," << subjet_iter->
                bDiscriminator("combinedSecondaryVertexBJetTags") << "\n";
        }
    ...
    for (edm::View < pat::Jet >::const_iterator filterjet_iter =
         filterjets.begin(); filterjet_iter != filterjets.end();
         ++filterjet_iter) {

        if (printJet) {
            std::cout << "FilterjetTagged Pt: " << filterjet_iter->
                pt() << " E,M,eta,phi,Btag: " << filterjet_iter->p4().
                E() << "," << filterjet_iter->p4().
                M() << "," << filterjet_iter->eta() << "," << filterjet_iter->
                phi() << "," << filterjet_iter->
                bDiscriminator("combinedSecondaryVertexBJetTags") << "\n";
        }
        ...
        if (runOnMC_) {
            ...
            if ((filterjet_iter->genParton())) {
                ...
                if ((filterjet_iter->genParton()->mother()))
                    fj.bestMCmomid =
                        filterjet_iter->genParton()->mother()->pdgId();
            }
            TLorentzVector gJp4;
            if (gJ) {
                gJp4.SetPtEtaPhiE(gJ->pt(), gJ->eta(), gJ->phi(),
                ...
                fj.bestMCp4 = gJp4;
                if (verbose_) {
                    std::clog << "filter genJet matched Pt = " << gJp4.
                        Pt() << std::endl;
                    std::clog << "filter genJet matched eta = " << gJp4.
                        Eta() << std::endl;
                    std::clog << "filter genJet matched deltaR = " << gJp4.
                        DeltaR(fj.p4) << std::endl;
                    std::clog << "filter genJet matched mother id = " << fj.
                        bestMCmomid << std::endl;
                }
            }
        }
    ...
    if (metsCh.size()) {
        ...
        if (verbose_)
            std::cout << " METCharged " << hbbInfo->metCh.
                metSig << " " << hbbInfo->metCh.sumEt << std::endl;
    }
    // type 1 corr met
    ...
    if (pfmetsType1corr.size()) {
        ...
        hbbInfo->pfmetType1corr.metSig =
            metSignificance(&(pfmetsType1corr[0]));
        hbbInfo->pfmetType1corr.eLong = (pfmetsType1corr[0]).e_longitudinal();
        hbbInfo->pfmetType1corr.p4 = GENPTOLOR((pfmetsType1corr[0]));
        if (verbose_)
            std::cout << " type 1 corrected pfMET " << hbbInfo->
                pfmetType1corr.metSig << " " << hbbInfo->pfmetType1corr.
                sumEt << std::endl;
    }
    ...
    if (pfmetsType1p2corr.size()) {
        ...
        hbbInfo->pfmetType1p2corr.metSig =
            metSignificance(&(pfmetsType1p2corr[0]));
        hbbInfo->pfmetType1p2corr.eLong =
            (pfmetsType1p2corr[0]).e_longitudinal();
        hbbInfo->pfmetType1p2corr.p4 = GENPTOLOR((pfmetsType1p2corr[0]));
        if (verbose_)
            std::cout << " type 1 +2 corrected pfMET " << hbbInfo->
                pfmetType1p2corr.metSig << " " << hbbInfo->pfmetType1p2corr.
                sumEt << std::endl;
    }
...
*/
    ...
    edm::Handle < edm::View < reco::MET >
        >patType1CorrectedPFMetElectronEnDownHandle;
    iEvent.getByLabel("patType1CorrectedPFMetElectronEnDown",
                      patType1CorrectedPFMetElectronEnDownHandle);
    edm::View < reco::MET > patType1CorrectedPFMetsElectronEnDown =
        *patType1CorrectedPFMetElectronEnDownHandle;
    if (patType1CorrectedPFMetsElectronEnDown.size()) {
        metunc.sumEt = (patType1CorrectedPFMetsElectronEnDown[0]).sumEt();
        metunc.metSig =
            metSignificance(&(patType1CorrectedPFMetsElectronEnDown[0]));
        metunc.eLong =
            (patType1CorrectedPFMetsElectronEnDown[0]).e_longitudinal();
        metunc.p4 = GENPTOLOR((patType1CorrectedPFMetsElectronEnDown[0]));
        hbbInfo->metUncInfo.push_back(metunc);
    }

    edm::Handle < edm::View < reco::MET >
        >patType1CorrectedPFMetElectronEnUpHandle;
    iEvent.getByLabel("patType1CorrectedPFMetElectronEnUp",
                      patType1CorrectedPFMetElectronEnUpHandle);
    edm::View < reco::MET > patType1CorrectedPFMetsElectronEnUp =
        *patType1CorrectedPFMetElectronEnUpHandle;
    if (patType1CorrectedPFMetsElectronEnUp.size()) {
        metunc.sumEt = (patType1CorrectedPFMetsElectronEnUp[0]).sumEt();
        metunc.metSig =
            metSignificance(&(patType1CorrectedPFMetsElectronEnUp[0]));
        metunc.eLong =
            (patType1CorrectedPFMetsElectronEnUp[0]).e_longitudinal();
    ...
    edm::Handle < edm::View < reco::MET >
        >patType1CorrectedPFMetMuonEnDownHandle;
    iEvent.getByLabel("patType1CorrectedPFMetMuonEnDown",
                      patType1CorrectedPFMetMuonEnDownHandle);
    edm::View < reco::MET > patType1CorrectedPFMetsMuonEnDown =
        *patType1CorrectedPFMetMuonEnDownHandle;
    if (patType1CorrectedPFMetsMuonEnDown.size()) {
        metunc.sumEt = (patType1CorrectedPFMetsMuonEnDown[0]).sumEt();
        metunc.metSig =
            metSignificance(&(patType1CorrectedPFMetsMuonEnDown[0]));
        metunc.eLong =
            (patType1CorrectedPFMetsMuonEnDown[0]).e_longitudinal();
        metunc.p4 = GENPTOLOR((patType1CorrectedPFMetsMuonEnDown[0]));
        hbbInfo->metUncInfo.push_back(metunc);
    }

    edm::Handle < edm::View < reco::MET >
        >patType1CorrectedPFMetMuonEnUpHandle;
    iEvent.getByLabel("patType1CorrectedPFMetMuonEnUp",
                      patType1CorrectedPFMetMuonEnUpHandle);
    edm::View < reco::MET > patType1CorrectedPFMetsMuonEnUp =
        *patType1CorrectedPFMetMuonEnUpHandle;
    if (patType1CorrectedPFMetsMuonEnUp.size()) {
        metunc.sumEt = (patType1CorrectedPFMetsMuonEnUp[0]).sumEt();
        metunc.metSig =
            metSignificance(&(patType1CorrectedPFMetsMuonEnUp[0]));
    ...
    edm::Handle < edm::View < reco::MET >
        >patType1CorrectedPFMetTauEnDownHandle;
    iEvent.getByLabel("patType1CorrectedPFMetTauEnDown",
                      patType1CorrectedPFMetTauEnDownHandle);
    edm::View < reco::MET > patType1CorrectedPFMetsTauEnDown =
        *patType1CorrectedPFMetTauEnDownHandle;
    if (patType1CorrectedPFMetsTauEnDown.size()) {
        metunc.sumEt = (patType1CorrectedPFMetsTauEnDown[0]).sumEt();
        metunc.metSig =
            metSignificance(&(patType1CorrectedPFMetsTauEnDown[0]));
    ...
    edm::Handle < edm::View < reco::MET >
        >patType1CorrectedPFMetTauEnUpHandle;
    iEvent.getByLabel("patType1CorrectedPFMetTauEnUp",
                      patType1CorrectedPFMetTauEnUpHandle);
    edm::View < reco::MET > patType1CorrectedPFMetsTauEnUp =
        *patType1CorrectedPFMetTauEnUpHandle;
    ...
    edm::Handle < edm::View < reco::MET >
        >patType1CorrectedPFMetJetEnDownHandle;
    iEvent.getByLabel("patType1CorrectedPFMetJetEnDown",
                      patType1CorrectedPFMetJetEnDownHandle);
    edm::View < reco::MET > patType1CorrectedPFMetsJetEnDown =
        *patType1CorrectedPFMetJetEnDownHandle;
    if (patType1CorrectedPFMetsJetEnDown.size()) {
        metunc.sumEt = (patType1CorrectedPFMetsJetEnDown[0]).sumEt();
        metunc.metSig =
            metSignificance(&(patType1CorrectedPFMetsJetEnDown[0]));
    ...
    edm::Handle < edm::View < reco::MET >
        >patType1CorrectedPFMetJetEnUpHandle;
    iEvent.getByLabel("patType1CorrectedPFMetJetEnUp",
                      patType1CorrectedPFMetJetEnUpHandle);
    edm::View < reco::MET > patType1CorrectedPFMetsJetEnUp =
        *patType1CorrectedPFMetJetEnUpHandle;
    ...
    edm::Handle < edm::View < reco::MET >
        >patType1CorrectedPFMetJetResDownHandle;
    iEvent.getByLabel("patType1CorrectedPFMetJetResDown",
                      patType1CorrectedPFMetJetResDownHandle);
    edm::View < reco::MET > patType1CorrectedPFMetsJetResDown =
        *patType1CorrectedPFMetJetResDownHandle;
    if (patType1CorrectedPFMetsJetResDown.size()) {
        metunc.sumEt = (patType1CorrectedPFMetsJetResDown[0]).sumEt();
        metunc.metSig =
            metSignificance(&(patType1CorrectedPFMetsJetResDown[0]));
        metunc.eLong =
            (patType1CorrectedPFMetsJetResDown[0]).e_longitudinal();
        metunc.p4 = GENPTOLOR((patType1CorrectedPFMetsJetResDown[0]));
        hbbInfo->metUncInfo.push_back(metunc);
    }

    edm::Handle < edm::View < reco::MET >
        >patType1CorrectedPFMetJetResUpHandle;
    iEvent.getByLabel("patType1CorrectedPFMetJetResUp",
                      patType1CorrectedPFMetJetResUpHandle);
    edm::View < reco::MET > patType1CorrectedPFMetsJetResUp =
        *patType1CorrectedPFMetJetResUpHandle;
    if (patType1CorrectedPFMetsJetResUp.size()) {
        metunc.sumEt = (patType1CorrectedPFMetsJetResUp[0]).sumEt();
        metunc.metSig =
            metSignificance(&(patType1CorrectedPFMetsJetResUp[0]));
    ...
    edm::Handle < edm::View < reco::MET >
        >patType1CorrectedPFMetUnclusteredEnDownHandle;
    iEvent.getByLabel("patType1CorrectedPFMetUnclusteredEnDown",
                      patType1CorrectedPFMetUnclusteredEnDownHandle);
    edm::View < reco::MET > patType1CorrectedPFMetsUnclusteredEnDown =
        *patType1CorrectedPFMetUnclusteredEnDownHandle;
    if (patType1CorrectedPFMetsUnclusteredEnDown.size()) {
        metunc.sumEt = (patType1CorrectedPFMetsUnclusteredEnDown[0]).sumEt();
        metunc.metSig =
            metSignificance(&(patType1CorrectedPFMetsUnclusteredEnDown[0]));
        metunc.eLong =
            (patType1CorrectedPFMetsUnclusteredEnDown[0]).e_longitudinal();
        metunc.p4 = GENPTOLOR((patType1CorrectedPFMetsUnclusteredEnDown[0]));
        hbbInfo->metUncInfo.push_back(metunc);
    }

    edm::Handle < edm::View < reco::MET >
        >patType1CorrectedPFMetUnclusteredEnUpHandle;
    iEvent.getByLabel("patType1CorrectedPFMetUnclusteredEnUp",
                      patType1CorrectedPFMetUnclusteredEnUpHandle);
    edm::View < reco::MET > patType1CorrectedPFMetsUnclusteredEnUp =
        *patType1CorrectedPFMetUnclusteredEnUpHandle;
    if (patType1CorrectedPFMetsUnclusteredEnUp.size()) {
        metunc.sumEt = (patType1CorrectedPFMetsUnclusteredEnUp[0]).sumEt();
        metunc.metSig =
            metSignificance(&(patType1CorrectedPFMetsUnclusteredEnUp[0]));
        metunc.eLong =
            (patType1CorrectedPFMetsUnclusteredEnUp[0]).e_longitudinal();
    ...
    edm::Handle < edm::View < reco::MET >
        >patType1p2CorrectedPFMetElectronEnDownHandle;
    iEvent.getByLabel("patType1p2CorrectedPFMetElectronEnDown",
                      patType1p2CorrectedPFMetElectronEnDownHandle);
    edm::View < reco::MET > patType1p2CorrectedPFMetsElectronEnDown =
        *patType1p2CorrectedPFMetElectronEnDownHandle;
    if (patType1p2CorrectedPFMetsElectronEnDown.size()) {
        metunc.sumEt = (patType1p2CorrectedPFMetsElectronEnDown[0]).sumEt();
        metunc.metSig =
            metSignificance(&(patType1p2CorrectedPFMetsElectronEnDown[0]));
        metunc.eLong =
            (patType1p2CorrectedPFMetsElectronEnDown[0]).e_longitudinal();
        metunc.p4 = GENPTOLOR((patType1p2CorrectedPFMetsElectronEnDown[0]));
        hbbInfo->metUncInfo.push_back(metunc);
    }

    edm::Handle < edm::View < reco::MET >
        >patType1p2CorrectedPFMetElectronEnUpHandle;
    iEvent.getByLabel("patType1p2CorrectedPFMetElectronEnUp",
                      patType1p2CorrectedPFMetElectronEnUpHandle);
    edm::View < reco::MET > patType1p2CorrectedPFMetsElectronEnUp =
        *patType1p2CorrectedPFMetElectronEnUpHandle;
    if (patType1p2CorrectedPFMetsElectronEnUp.size()) {
        metunc.sumEt = (patType1p2CorrectedPFMetsElectronEnUp[0]).sumEt();
        metunc.metSig =
            metSignificance(&(patType1p2CorrectedPFMetsElectronEnUp[0]));
        metunc.eLong =
            (patType1p2CorrectedPFMetsElectronEnUp[0]).e_longitudinal();
    ...
    edm::Handle < edm::View < reco::MET >
        >patType1p2CorrectedPFMetMuonEnDownHandle;
    iEvent.getByLabel("patType1p2CorrectedPFMetMuonEnDown",
                      patType1p2CorrectedPFMetMuonEnDownHandle);
    edm::View < reco::MET > patType1p2CorrectedPFMetsMuonEnDown =
        *patType1p2CorrectedPFMetMuonEnDownHandle;
    if (patType1p2CorrectedPFMetsMuonEnDown.size()) {
        metunc.sumEt = (patType1p2CorrectedPFMetsMuonEnDown[0]).sumEt();
        metunc.metSig =
            metSignificance(&(patType1p2CorrectedPFMetsMuonEnDown[0]));
        metunc.eLong =
            (patType1p2CorrectedPFMetsMuonEnDown[0]).e_longitudinal();
        metunc.p4 = GENPTOLOR((patType1p2CorrectedPFMetsMuonEnDown[0]));
        hbbInfo->metUncInfo.push_back(metunc);
    }

    edm::Handle < edm::View < reco::MET >
        >patType1p2CorrectedPFMetMuonEnUpHandle;
    iEvent.getByLabel("patType1p2CorrectedPFMetMuonEnUp",
                      patType1p2CorrectedPFMetMuonEnUpHandle);
    edm::View < reco::MET > patType1p2CorrectedPFMetsMuonEnUp =
        *patType1p2CorrectedPFMetMuonEnUpHandle;
    if (patType1p2CorrectedPFMetsMuonEnUp.size()) {
        metunc.sumEt = (patType1p2CorrectedPFMetsMuonEnUp[0]).sumEt();
        metunc.metSig =
            metSignificance(&(patType1p2CorrectedPFMetsMuonEnUp[0]));
        metunc.eLong =
            (patType1p2CorrectedPFMetsMuonEnUp[0]).e_longitudinal();
    ...
    edm::Handle < edm::View < reco::MET >
        >patType1p2CorrectedPFMetTauEnDownHandle;
    iEvent.getByLabel("patType1p2CorrectedPFMetTauEnDown",
                      patType1p2CorrectedPFMetTauEnDownHandle);
    edm::View < reco::MET > patType1p2CorrectedPFMetsTauEnDown =
        *patType1p2CorrectedPFMetTauEnDownHandle;
    if (patType1p2CorrectedPFMetsTauEnDown.size()) {
        metunc.sumEt = (patType1p2CorrectedPFMetsTauEnDown[0]).sumEt();
        metunc.metSig =
            metSignificance(&(patType1p2CorrectedPFMetsTauEnDown[0]));
        metunc.eLong =
            (patType1p2CorrectedPFMetsTauEnDown[0]).e_longitudinal();
        metunc.p4 = GENPTOLOR((patType1p2CorrectedPFMetsTauEnDown[0]));
        hbbInfo->metUncInfo.push_back(metunc);
    }

    edm::Handle < edm::View < reco::MET >
        >patType1p2CorrectedPFMetTauEnUpHandle;
    iEvent.getByLabel("patType1p2CorrectedPFMetTauEnUp",
                      patType1p2CorrectedPFMetTauEnUpHandle);
    edm::View < reco::MET > patType1p2CorrectedPFMetsTauEnUp =
        *patType1p2CorrectedPFMetTauEnUpHandle;
    if (patType1p2CorrectedPFMetsTauEnUp.size()) {
        metunc.sumEt = (patType1p2CorrectedPFMetsTauEnUp[0]).sumEt();
        metunc.metSig =
            metSignificance(&(patType1p2CorrectedPFMetsTauEnUp[0]));
    ...
    edm::Handle < edm::View < reco::MET >
        >patType1p2CorrectedPFMetJetEnDownHandle;
    iEvent.getByLabel("patType1p2CorrectedPFMetJetEnDown",
                      patType1p2CorrectedPFMetJetEnDownHandle);
    edm::View < reco::MET > patType1p2CorrectedPFMetsJetEnDown =
        *patType1p2CorrectedPFMetJetEnDownHandle;
    if (patType1p2CorrectedPFMetsJetEnDown.size()) {
        metunc.sumEt = (patType1p2CorrectedPFMetsJetEnDown[0]).sumEt();
        metunc.metSig =
            metSignificance(&(patType1p2CorrectedPFMetsJetEnDown[0]));
        metunc.eLong =
            (patType1p2CorrectedPFMetsJetEnDown[0]).e_longitudinal();
        metunc.p4 = GENPTOLOR((patType1p2CorrectedPFMetsJetEnDown[0]));
        hbbInfo->metUncInfo.push_back(metunc);
    }

    edm::Handle < edm::View < reco::MET >
        >patType1p2CorrectedPFMetJetEnUpHandle;
    iEvent.getByLabel("patType1p2CorrectedPFMetJetEnUp",
                      patType1p2CorrectedPFMetJetEnUpHandle);
    edm::View < reco::MET > patType1p2CorrectedPFMetsJetEnUp =
        *patType1p2CorrectedPFMetJetEnUpHandle;
    if (patType1p2CorrectedPFMetsJetEnUp.size()) {
        metunc.sumEt = (patType1p2CorrectedPFMetsJetEnUp[0]).sumEt();
        metunc.metSig =
            metSignificance(&(patType1p2CorrectedPFMetsJetEnUp[0]));
    ...
    edm::Handle < edm::View < reco::MET >
        >patType1p2CorrectedPFMetJetResDownHandle;
    iEvent.getByLabel("patType1p2CorrectedPFMetJetResDown",
                      patType1p2CorrectedPFMetJetResDownHandle);
    edm::View < reco::MET > patType1p2CorrectedPFMetsJetResDown =
        *patType1p2CorrectedPFMetJetResDownHandle;
    if (patType1p2CorrectedPFMetsJetResDown.size()) {
        metunc.sumEt = (patType1p2CorrectedPFMetsJetResDown[0]).sumEt();
        metunc.metSig =
            metSignificance(&(patType1p2CorrectedPFMetsJetResDown[0]));
        metunc.eLong =
            (patType1p2CorrectedPFMetsJetResDown[0]).e_longitudinal();
        metunc.p4 = GENPTOLOR((patType1p2CorrectedPFMetsJetResDown[0]));
        hbbInfo->metUncInfo.push_back(metunc);
    }

    edm::Handle < edm::View < reco::MET >
        >patType1p2CorrectedPFMetJetResUpHandle;
    iEvent.getByLabel("patType1p2CorrectedPFMetJetResUp",
                      patType1p2CorrectedPFMetJetResUpHandle);
    edm::View < reco::MET > patType1p2CorrectedPFMetsJetResUp =
        *patType1p2CorrectedPFMetJetResUpHandle;
    if (patType1p2CorrectedPFMetsJetResUp.size()) {
        metunc.sumEt = (patType1p2CorrectedPFMetsJetResUp[0]).sumEt();
        metunc.metSig =
            metSignificance(&(patType1p2CorrectedPFMetsJetResUp[0]));
        metunc.eLong =
            (patType1p2CorrectedPFMetsJetResUp[0]).e_longitudinal();
    ...
    edm::Handle < edm::View < reco::MET >
        >patType1p2CorrectedPFMetUnclusteredEnDownHandle;
    iEvent.getByLabel("patType1p2CorrectedPFMetUnclusteredEnDown",
                      patType1p2CorrectedPFMetUnclusteredEnDownHandle);
    edm::View < reco::MET > patType1p2CorrectedPFMetsUnclusteredEnDown =
        *patType1p2CorrectedPFMetUnclusteredEnDownHandle;
    if (patType1p2CorrectedPFMetsUnclusteredEnDown.size()) {
        metunc.sumEt =
            (patType1p2CorrectedPFMetsUnclusteredEnDown[0]).sumEt();
        metunc.metSig =
            metSignificance(&(patType1p2CorrectedPFMetsUnclusteredEnDown[0]));
        metunc.eLong =
            (patType1p2CorrectedPFMetsUnclusteredEnDown[0]).e_longitudinal();
        metunc.p4 =
            GENPTOLOR((patType1p2CorrectedPFMetsUnclusteredEnDown[0]));
        hbbInfo->metUncInfo.push_back(metunc);
    }

    edm::Handle < edm::View < reco::MET >
        >patType1p2CorrectedPFMetUnclusteredEnUpHandle;
    iEvent.getByLabel("patType1p2CorrectedPFMetUnclusteredEnUp",
                      patType1p2CorrectedPFMetUnclusteredEnUpHandle);
    edm::View < reco::MET > patType1p2CorrectedPFMetsUnclusteredEnUp =
        *patType1p2CorrectedPFMetUnclusteredEnUpHandle;
    if (patType1p2CorrectedPFMetsUnclusteredEnUp.size()) {
        metunc.sumEt = (patType1p2CorrectedPFMetsUnclusteredEnUp[0]).sumEt();
        metunc.metSig =
            metSignificance(&(patType1p2CorrectedPFMetsUnclusteredEnUp[0]));
        metunc.eLong =
            (patType1p2CorrectedPFMetsUnclusteredEnUp[0]).e_longitudinal();
    ...
    if (metsTC.size()) {
        ...
        if (verbose_)
            std::cout << " METTC " << hbbInfo->tcmet.
                metSig << " " << hbbInfo->tcmet.sumEt << std::endl;
    }
    ...
    if (metspfMETNoPU.size()) {
        ...
        if (verbose_)
            std::cout << " pfMETNoPU " << hbbInfo->metNoPU.
                metSig << " " << hbbInfo->metNoPU.sumEt << std::endl;
    }
    ...
    if (metsHT.size()) {
        ...
        if (verbose_)
            std::cout << " METHT " << hbbInfo->mht.metSig << " " << hbbInfo->
                mht.sumEt << std::endl;
    }
    ...
    if (mets.size()) {
        ...
        if (verbose_)
            std::cout << " METCALO " << hbbInfo->calomet.
                metSig << " " << hbbInfo->calomet.sumEt << std::endl;
    }
    ...
    if (metsPF.size()) {
        ...
        if (verbose_)
            std::cout << " METPF " << hbbInfo->pfmet.
                metSig << " " << hbbInfo->pfmet.sumEt << std::endl;
    }


    if (verbose_) {
        std::cout << "METs: calomet " << mets.size() << " tcmet" << metsTC.
            size() << " pfmet " << metsPF.size() << " MHT" << metsHT.
            size() << std::endl;
    }
    ...
    for (edm::View < pat::Muon >::const_iterator mu = muons.begin();
        ...
        Geom::Phi <
            double >deltaphi(mu->phi() - atan2(mf.p4.Px(), mf.p4.Py()));
        ...
        for (int itrig = 0; itrig != ntrigs; ++itrig) {
            ...
            if ((mu->triggerObjectMatchesByPath(trigName, false, false).
                 size() != 0)) {
                ...
                if (verbose_) {
                    ...
                    std::
                        ...
                        std::endl;
                    std::
                        clog << "Matching parameters are defined in the cfg"
                        << std::endl;
                    std::clog << "Trigger bit = " << itrig << std::endl;
                    std::clog << "Trigger name = " << trigName << std::endl;
                    std::
                        clog << "Trigger object matched collection size = " <<
                        mu->triggerObjectMatchesByPath(trigName, false,
                                                       false).
                        size() << std::endl;
                    std::clog << "Pat Muon pt = " << mf.p4.
                        Pt() << " HLT object matched = " << mu->
                        triggerObjectMatch(0)->p4().Pt() << std::endl;
                    std::
                        clog <<
                        "+++++++++++++++++++++++++++++++++++++++++++++++++" <<
                        std::endl;
        ...
        if (runOnMC_) {
            ...
            if (muMc != 0) {
                ...
                if (muMc->mother() != 0)
                    mf.mcMomId = muMc->mother()->pdgId();
                if (muMc->mother() != 0 && muMc->mother()->mother() != 0)
                    mf.mcgMomId = muMc->mother()->mother()->pdgId();
            }
        }
    ...
    EcalClusterLazyTools lazyTools(iEvent, iSetup, reducedEBRecHitCollection,
...
/* Check if there are photons sharing the super cluster*/
        for (size_t k = 0; k < photonsForIso.size(); k++) {
            if (deltaR
                (elec->eta(), elec->phi(), photonsForIso[k].eta(),
                 photonsForIso[k].phi()) < 0.05
                && abs(photonsForIso[k].pt() -
                       elec->pt()) / (photonsForIso[k].pt() + elec->pt()) <
                0.05) {
        ...
        Geom::Phi <
            double >deltaphi(elec->superCluster()->phi() -
                             atan2(hbbInfo->pfmet.p4.Py(),
                                   hbbInfo->pfmet.p4.Px()));
        ...
        if (elec->gsfTrack().isNonnull()) {
            ef.innerHits =
                elec->gsfTrack()->trackerExpectedHitsInner().numberOfHits();
        }
        ef.isEB = elec->isEB();
        ef.isEE = elec->isEE();
/* 2012 ELEID*/
        ...
        std::vector < float >vCov =
            lazyTools.localCovariances(*(ele.superCluster()->seed()));
        if (!isnan(vCov[2]))
            ef.fMVAVar_spp = sqrt(vCov[2]);     //EleSigmaIPhiIPhi
        else
            ef.fMVAVar_spp = 0.;
        ...
        ef.fMVAVar_R9 =
            lazyTools.e3x3(*(ele.superCluster()->seed())) /
        ...
        if (ele.gsfTrack().isNonnull()) {
            ...
            ef.fMVAVar_d0 =
                (-1.0) * ele.closestCtfTrackRef()->dxy(vertex.position());
        } else {
            ...
            // fMVAVar_ip3dSig = 0.0;
            if (ele.gsfTrack().isNonnull()) {
                const double gsfsign =
                    ((-ele.gsfTrack()->dxy(vertex.position())) >=
                     0) ? 1. : -1.;

                const reco::TransientTrack & tt =
                    transientTrackBuilder.build(ele.gsfTrack());
                const std::pair < bool, Measurement1D > &ip3dpv =
                    IPTools::absoluteImpactParameter3D(tt, vertex);
                if (ip3dpv.first) {
...
*/
        ...
        for (int itrig = 0; itrig != ntrigs; ++itrig) {
            ...
            if ((elec->triggerObjectMatchesByPath(trigName).size() != 0)) {
                ...
                if (verbose_) {
                    ...
                    std::
                        clog << "Trigger object matched collection size = " <<
                        elec->triggerObjectMatchesByPath(trigName).
                        size() << std::endl;
                    std::clog << "Pat Electron pt = " << ef.p4.
                        Pt() << " HLT object matched = " << elec->
                        triggerObjectMatch(0)->p4().Pt() << std::endl;
                    std::
                        clog <<
                        "+++++++++++++++++++++++++++++++++++++++++++++++++" <<
        ...
        if (runOnMC_) {
            ...
            if (elecMc != 0) {
                ...
                if (elecMc->mother() != 0)
                    ef.mcMomId = elecMc->mother()->pdgId();
                if (elecMc->mother() != 0 && elecMc->mother()->mother() != 0)
                    ef.mcgMomId = elecMc->mother()->mother()->pdgId();
            }
        }
    ...
    for (edm::View < pat::Tau >::const_iterator tau = taus.begin();
         tau != taus.end(); ++tau) {
        ...
        if (tau->isTauIDAvailable("againstElectronMedium"))
            tf.againstElectronMedium = tau->tauID("againstElectronMedium");
        if (tau->isTauIDAvailable("againstElectronTight"))
            tf.againstElectronTight = tau->tauID("againstElectronTight");
        if (tau->isTauIDAvailable("againstMuonLoose"))
            tf.againstMuonLoose = tau->tauID("againstMuonLoose");
        if (tau->isTauIDAvailable("againstMuonTight"))
            tf.againstMuonTight = tau->tauID("againstMuonTight");
        if (tau->isTauIDAvailable("byLooseIsolation"))
            tf.byLooseIsolation = tau->tauID("byLooseIsolation");
        if (tau->isTauIDAvailable("byMediumIsolation"))
            tf.byMediumIsolation = tau->tauID("byMediumIsolation");
        if (tau->isTauIDAvailable("byTightIsolation"))
            tf.byTightIsolation = tau->tauID("byTightIsolation");
        if (tau->isTauIDAvailable("byVLooseIsolation"))
            tf.byVLooseIsolation = tau->tauID("byVLooseIsolation");
        if (tau->isTauIDAvailable("decayModeFinding"))
            tf.decayModeFinding = tau->tauID("decayModeFinding");
        if (tau->isTauIDAvailable("byIsolation"))
            tf.byIsolation = tau->tauID("byIsolation");
        if (tau->isTauIDAvailable("trackIsolation"))
            tf.trackIsolation = tau->tauID("trackIsolation");
        if (tau->isTauIDAvailable("byTaNCfrOnePercent"))
            tf.byTaNCfrOnePercent = tau->tauID("byTaNCfrOnePercent");
        if (tau->isTauIDAvailable("byTaNCfrHalfPercent"))
            tf.byTaNCfrHalfPercent = tau->tauID("byTaNCfrHalfPercent");
        if (tau->isTauIDAvailable("byTaNCfrQuarterPercent"))
            tf.byTaNCfrQuarterPercent = tau->tauID("byTaNCfrQuarterPercent");
        if (tau->isTauIDAvailable("byTaNCfrTenthPercent"))
            tf.byTaNCfrTenthPercent = tau->tauID("byTaNCfrTenthPercent");
        if (tau->isTauIDAvailable("byTaNC"))
            tf.byTaNC = tau->tauID("byTaNC");
        if (tau->isTauIDAvailable("byLooseCombinedIsolationDeltaBetaCorr"))
            tf.byLooseCombinedIsolationDeltaBetaCorr =
                tau->tauID("byLooseCombinedIsolationDeltaBetaCorr");
        if (tau->isTauIDAvailable("againstElectronMVA"))
            tf.againstElectronMVA = tau->tauID("againstElectronMVA");
        if (tau->isPFTau()) {
            tf.isolationPFChargedHadrCandsPtSum =
                tau->isolationPFChargedHadrCandsPtSum();
            tf.isolationPFGammaCandsEtSum = tau->isolationPFGammaCandsEtSum();
            if (tau->leadPFChargedHadrCand().isAvailable())
                tf.leadPFChargedHadrCandPt =
                    tau->leadPFChargedHadrCand()->pt();
            tf.NsignalPFChargedHadrCands =
                tau->signalPFChargedHadrCands().size();
            tf.NsignalPFGammaCands = tau->signalPFGammaCands().size();
        }
        hbbInfo->tauInfo.push_back(tf);
        if (verbose_) {
            std::cout << "SCZ DEBUG: againstElectronLoose is " << tf.
                againstElectronLoose << std::endl;
            std::cout << "SCZ DEBUG: againstElectronMedium is " << tf.
                againstElectronMedium << std::endl;
            std::cout << "SCZ DEBUG: againstElectronTight is " << tf.
                againstElectronTight << std::endl;
            std::cout << "SCZ DEBUG: againstMuonLoose is " << tf.
                againstMuonLoose << std::endl;
            std::cout << "SCZ DEBUG: againstMuonTight is " << tf.
                againstMuonTight << std::endl;
            std::cout << "SCZ DEBUG: byLooseIsolation is " << tf.
                byLooseIsolation << std::endl;
            std::cout << "SCZ DEBUG: byMediumIsolation is " << tf.
                byMediumIsolation << std::endl;
            std::cout << "SCZ DEBUG: byTightIsolation is " << tf.
                byTightIsolation << std::endl;
            std::cout << "SCZ DEBUG: byVLooseIsolation is " << tf.
                byVLooseIsolation << std::endl;
            std::cout << "SCZ DEBUG: decayModeFinding is " << tf.
                decayModeFinding << std::endl;
            std::cout << "SCZ DEBUG: byIsolation is " << tf.
                byIsolation << std::endl;
            std::cout << "SCZ DEBUG: trackIsolation is " << tf.
                trackIsolation << std::endl;
            std::cout << "SCZ DEBUG: byTaNCfrOnePercent is " << tf.
                byTaNCfrOnePercent << std::endl;
            std::cout << "SCZ DEBUG: byTaNCfrHalfPercent is " << tf.
                byTaNCfrHalfPercent << std::endl;
            std::cout << "SCZ DEBUG: byTaNCfrQuarterPercent is " << tf.
                byTaNCfrQuarterPercent << std::endl;
            std::cout << "SCZ DEBUG: byTaNCfrTenthPercent is " << tf.
                byTaNCfrTenthPercent << std::endl;
            std::cout << "SCZ DEBUG: byTaNC is " << tf.byTaNC << std::endl;
            std::
                cout << "SCZ DEBUG: isolationPFChargedHadrCandsPtSum is " <<
                tf.isolationPFChargedHadrCandsPtSum << std::endl;
            std::cout << "SCZ DEBUG: isolationPFGammaCandsEtSum is " << tf.
                isolationPFGammaCandsEtSum << std::endl;
            std::cout << "SCZ DEBUG: isolationPFGammaCandsEtSum is " << tf.
                leadPFChargedHadrCandPt << std::endl;
            std::cout << "SCZ DEBUG: NsignalPFChargedHadrCands is " << tf.
                NsignalPFChargedHadrCands << std::endl;
            std::cout << "SCZ DEBUG: NsignalPFGammaCands is " << tf.
                NsignalPFGammaCands << std::endl;
            std::
                cout << "SCZ DEBUG: byLooseCombinedIsolationDeltaBetaCorr is "
                << tf.byLooseCombinedIsolationDeltaBetaCorr << std::endl;
            std::cout << "SCZ DEBUG: againstElectronMVA is " << tf.
                againstElectronMVA << std::endl;
        }
    }
    ...
    std::sort(hbbInfo->eleInfo.begin(), hbbInfo->eleInfo.end(),
              ptComparatorE);
    std::sort(hbbInfo->tauInfo.begin(), hbbInfo->tauInfo.end(),
              ptComparatorTau);
    ...
    if (verbose_) {
        ...
        std::cout << " SimpleJets1 = " << hbbInfo->simpleJets.
            size() << std::endl << " SimpleJets2 = " << hbbInfo->simpleJets2.
            size() << std::endl << " SubJets = " << hbbInfo->subJets.
            size() << std::endl << " HardJets = " << hbbInfo->hardJets.
            size() << std::endl << " FilterJets = " << hbbInfo->filterJets.
            size() << std::endl << " Muons = " << hbbInfo->muInfo.
            size() << std::endl << " Electrons = " << hbbInfo->eleInfo.
            size() << std::endl << " Taus = " << hbbInfo->tauInfo.
            size() << std::endl << " Electrons = " << hbbInfo->eleInfo.
...
void HbbAnalyzerNew::fillMuBlock(edm::View < pat::Muon >::const_iterator mu,
                                 int muInfo[15]) {
    if (muon::isGoodMuon(*mu, muon::TMLastStationLoose))
        muInfo[0] = 1;
    if (muon::isGoodMuon(*mu, muon::TMLastStationTight))
        muInfo[1] = 1;
    if (muon::isGoodMuon(*mu, muon::TM2DCompatibilityLoose))
        muInfo[2] = 1;
    if (muon::isGoodMuon(*mu, muon::TM2DCompatibilityTight))
        muInfo[3] = 1;
    if (muon::isGoodMuon(*mu, muon::TMOneStationLoose))
        muInfo[4] = 1;
    if (muon::isGoodMuon(*mu, muon::TMOneStationTight))
        muInfo[5] = 1;
    if (muon::isGoodMuon(*mu, muon::TMLastStationOptimizedLowPtLoose))
        muInfo[6] = 1;
    if (muon::isGoodMuon(*mu, muon::TMLastStationOptimizedLowPtTight))
        muInfo[7] = 1;
    if (muon::isGoodMuon(*mu, muon::TMOneStationAngLoose))
        muInfo[8] = 1;
    if (muon::isGoodMuon(*mu, muon::TMOneStationAngTight))
        muInfo[9] = 1;
    if (muon::isGoodMuon(*mu, muon::TMLastStationAngLoose))
        muInfo[10] = 1;
    if (muon::isGoodMuon(*mu, muon::TMLastStationAngTight))
        muInfo[11] = 1;
    if (muon::isGoodMuon(*mu, muon::GMTkChiCompatibility))
        muInfo[12] = 1;
    if (muon::isGoodMuon(*mu, muon::GMStaChiCompatibility))
        muInfo[13] = 1;
    if (muon::isGoodMuon(*mu, muon::GMTkKinkTight))
        muInfo[14] = 1;
}

// ------------ method called once each job just before starting event loop  ------------
void HbbAnalyzerNew::beginJob() {
}


// ------------ method called once each job just after ending the event loop  ------------
void HbbAnalyzerNew::endJob() {
}
...
//calculate TVector using only charged tracks
    for (size_t idx = 0; idx < patJetpfc.size(); idx++) {
        if (patJetpfc.at(idx)->charge() != 0) {
            ...
            r.Set(pi.Rapidity() - J.Rapidity(),
                  reco::deltaPhi(patJetpfc.at(idx)->phi(), J.Phi()));
            r_mag = r.Mod();
...
TLorentzVector HbbAnalyzerNew::getChargedTracksMomentum(const pat::Jet *
                                                        patJet) {
...
void HbbAnalyzerNew::fillScaleFactors(VHbbEvent::SimpleJet & sj,
    ...
    if (TMath::Abs(sj.flavour) == 4 or TMath::Abs(sj.flavour) == 5) {
        ...
        if (iSF.BTAGSF_CSVL->
            isResultOk(PerformanceResult::BTAGBEFFCORR, measurePoint)) {
            sj.SF_CSVL =
                iSF.BTAGSF_CSVL->getResult(PerformanceResult::BTAGBEFFCORR,
                                           measurePoint);
            sj.SF_CSVLerr =
                iSF.BTAGSF_CSVL->getResult(PerformanceResult::BTAGBERRCORR,
                                           measurePoint);
            if (verbose_) {
                std::clog << "C/B Jet flavour = " << sj.flavour << std::endl;
                std::clog << "C/B Jet Et = " << sj.p4.Et() << std::endl;
                std::clog << "C/B Jet eta = " << sj.p4.Eta() << std::endl;
                std::clog << "C/B CSVL Scale Factor = " << sj.
                    SF_CSVL << std::endl;
                std::clog << "C/B CSVL Scale Factor error = " << sj.
                    SF_CSVLerr << std::endl;
            }
        }
        if (iSF.BTAGSF_CSVM->
            isResultOk(PerformanceResult::BTAGBEFFCORR, measurePoint)) {
            sj.SF_CSVM =
                iSF.BTAGSF_CSVM->getResult(PerformanceResult::BTAGBEFFCORR,
                                           measurePoint);
            sj.SF_CSVMerr =
                iSF.BTAGSF_CSVM->getResult(PerformanceResult::BTAGBERRCORR,
                                           measurePoint);
        }
        if (iSF.BTAGSF_CSVT->
            isResultOk(PerformanceResult::BTAGBEFFCORR, measurePoint)) {
            sj.SF_CSVT =
                iSF.BTAGSF_CSVT->getResult(PerformanceResult::BTAGBEFFCORR,
                                           measurePoint);
            sj.SF_CSVTerr =
                iSF.BTAGSF_CSVT->getResult(PerformanceResult::BTAGBERRCORR,
                                           measurePoint);
        } else {
            if (verbose_) {
                std::
                    cerr << "No SF found in the database for this jet" <<
                    std::endl;
                std::clog << "No SF found: Jet flavour = " << sj.
                    flavour << std::endl;
                std::clog << "No SF found: Jet Et = " << sj.p4.
                    Et() << std::endl;
                std::clog << "No SF found: Jet eta = " << sj.p4.
                    Eta() << std::endl;
            }
        }
    } else {
        measurePoint.insert(BinningVariables::JetEt, sj.p4.Et());
        measurePoint.insert(BinningVariables::JetAbsEta, fabs(sj.p4.Eta()));
        if (iSF.MISTAGSF_CSVL->
            isResultOk(PerformanceResult::BTAGLEFFCORR, measurePoint)) {
            sj.SF_CSVL =
                iSF.MISTAGSF_CSVL->getResult(PerformanceResult::BTAGLEFFCORR,
                                             measurePoint);
            sj.SF_CSVLerr =
                iSF.MISTAGSF_CSVL->getResult(PerformanceResult::BTAGLERRCORR,
                                             measurePoint);
            if (verbose_) {
                std::clog << "Light Jet flavour = " << sj.
                    flavour << std::endl;
                std::clog << "Light Jet Et = " << sj.p4.Et() << std::endl;
                std::clog << "Light Jet eta = " << sj.p4.Eta() << std::endl;
                std::clog << "Light CSVL Scale Factor = " << sj.
                    SF_CSVL << std::endl;
                std::clog << "Light CSVL Scale Factor error = " << sj.
                    SF_CSVLerr << std::endl;
            }
        }
        if (iSF.MISTAGSF_CSVM->
            isResultOk(PerformanceResult::BTAGLEFFCORR, measurePoint)) {
            sj.SF_CSVM =
                iSF.MISTAGSF_CSVM->getResult(PerformanceResult::BTAGLEFFCORR,
                                             measurePoint);
            sj.SF_CSVMerr =
                iSF.MISTAGSF_CSVM->getResult(PerformanceResult::BTAGLERRCORR,
                                             measurePoint);
        }
        if (iSF.MISTAGSF_CSVT->
            isResultOk(PerformanceResult::BTAGLEFFCORR, measurePoint)) {
            sj.SF_CSVT =
                iSF.MISTAGSF_CSVT->getResult(PerformanceResult::BTAGLEFFCORR,
                                             measurePoint);
            sj.SF_CSVTerr =
                iSF.MISTAGSF_CSVT->getResult(PerformanceResult::BTAGLERRCORR,
                                             measurePoint);
        } else {
            if (verbose_) {
                std::
...
void HbbAnalyzerNew::setJecUnc(VHbbEvent::SimpleJet & sj,
...
void HbbAnalyzerNew::fillSimpleJet(VHbbEvent::SimpleJet & sj,
                                   edm::View <
                                   pat::Jet >::const_iterator jet_iter) {
    sj.flavour = jet_iter->partonFlavour();
    ...
    sj.jpb = jet_iter->bDiscriminator("jetBProbabilityBJetTags");
    sj.ssvhe =
        jet_iter->bDiscriminator("simpleSecondaryVertexHighEffBJetTags");
    sj.csv = jet_iter->bDiscriminator("combinedSecondaryVertexBJetTags");
    sj.csvmva =
        jet_iter->bDiscriminator("combinedSecondaryVertexMVABJetTags");
    sj.csvivf =
        jet_iter->bDiscriminator("combinedInclusiveSecondaryVertexBJetTags");
    ...
    const reco::SecondaryVertexTagInfo * tf =
        jet_iter->tagInfoSecondaryVertex();
    if (tf) {
        ...
        if (tf->nVertices() > 0) {
            sj.vtxPosition =
                TVector3(tf->secondaryVertex(0).position().x(),
                         tf->secondaryVertex(0).position().y(),
                         tf->secondaryVertex(0).position().z());
            sj.vtxMass = tf->secondaryVertex(0).p4().mass();
            sj.vtxNTracks = tf->secondaryVertex(0).nTracks();
            std::vector < reco::TrackBaseRef >::const_iterator tit =
                tf->secondaryVertex(0).tracks_begin();
    ...
    // CSV track info
    const reco::SecondaryVertexTagInfo * svTagInfos =
        jet_iter->tagInfoSecondaryVertex();
    ...
    tagInfos.push_back(dynamic_cast <
                       const reco::BaseTagInfo * >(ipTagInfos));
    tagInfos.push_back(dynamic_cast <
                       const reco::BaseTagInfo * >(svTagInfos));
    ...
    for (reco::TaggingVariableList::const_iterator iter = varList.begin();
        ...
        for (edm::RefVector < reco::TrackCollection >::const_iterator t =
            ...
            if (strcmp
                (reco::TaggingVariableTokens[iter->first],
                 "trackMomentum") == 0
                && (fabs((float) iter->second - (float) (*t)->p()) <
                    0.0001)) {
    ...
    sj.ptLeadTrack = -9999.;
    if (jet_iter->isPFJet() == true) {
        std::vector < reco::PFCandidatePtr > constituents =
            jet_iter->getPFConstituents();
        for (unsigned ic = 0; ic < constituents.size(); ++ic) {
            ...
            if (trackRef.isNonnull()) {
                if (trackRef->pt() > sj.ptLeadTrack)
                    sj.ptLeadTrack = trackRef->pt();
            }
        }
    }
...
float HbbAnalyzerNew::metSignificance(const reco::MET * met) {
    ...
    try {
        ...
        std::
            cout << "PROBLEM WITH MET SIGNIFICANCE sigma X2 and Y2 are: " <<
            sigmaX2 << " " << sigmaY2 << std::endl;
    }
    return significance;
}
