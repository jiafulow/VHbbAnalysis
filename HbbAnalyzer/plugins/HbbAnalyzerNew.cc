...
fatjetLabel_(iConfig.getParameter < edm::InputTag > ("fatjetTag")),
...
void HbbAnalyzerNew::produce(edm::Event & iEvent,
    ...
    computer = dynamic_cast < const GenericMVAJetTagComputer *>(csvCompHandle.product());
    ...
    const reco::Vertex & RecVtx = (*recVtxs)[VtxIn];
    const reco::Vertex & RecVtxFirst = (*recVtxs)[0];
    const reco::Vertex & vertex = RecVtxFirst;  // used in 2012 EleID
    ...
    if (runOnMC_) {
        ...
        if (puHandle.isValid()) {
            for (std::vector < PileupSummaryInfo >::const_iterator it =
                 puHandle->begin(); it != puHandle->end(); ++it) {
    ...
    if (runOnMC_) {
        ...
        for (size_t i = 0; i < genParticles->size(); ++i) {
            ...
            if (id == 25) {  // H
                ...
                ptemp.charge = p.charge();
                if (p.mother(0) != 0)
                    ptemp.momid = p.mother(0)->pdgId();
                if (p.mother(0) != 0 && p.mother(0)->mother(0) != 0)
                    ptemp.gmomid = p.mother(0)->mother(0)->pdgId();
                ptemp.p4 = GENPTOLOR(p);

                int ndau = p.numberOfDaughters();
                for (int j = 0; j < ndau; ++j) {
                    const reco::Candidate *dau = p.daughter(j);
                    ptemp.dauid.push_back(dau->pdgId());
                    ptemp.dauFourMomentum.push_back(GENPTOLORP(dau));
                }
                auxInfo->mcH.push_back(ptemp);
    ...
    bool printJet = false;
    for (edm::View < pat::Jet >::const_iterator jet_iter = fatjets.begin();
         jet_iter != fatjets.end(); ++jet_iter) {

        if (printJet)
            std::cout << "Jet Pt: " << jet_iter->pt() << " E,M: " << jet_iter->p4().E() << " " << jet_iter->p4().M() << std::endl;

        const reco::Jet::Constituents & constituents = jet_iter->getJetConstituents();
        //if(printJet)
        //    std::cout << "NsubJets: " << constituents.size() << std::endl;
    ...
    for (edm::View < pat::Jet >::const_iterator subjet_iter = subjets.begin();
        ...
        if (printJet)
            std::cout << "SubJetTagged Pt: " << subjet_iter->pt() << " E,M,eta,phi,Btag: " << subjet_iter->p4().E() << "," << subjet_iter->p4().M() << "," << subjet_iter->eta() << "," << subjet_iter->phi() << "," << subjet_iter->bDiscriminator("combinedSecondaryVertexBJetTags") << std::endl;
    ...
    /// Filter jets
    for (edm::View < pat::Jet >::const_iterator filterjet_iter = filterjets.begin();
         filterjet_iter != filterjets.end(); ++filterjet_iter) {

        if (printJet) {
            std::cout << "FilterjetTagged Pt: " << filterjet_iter->pt() << " E,M,eta,phi,Btag: " << filterjet_iter->p4().E() << "," << filterjet_iter->p4().M() << "," << filterjet_iter->eta() << "," << filterjet_iter->phi() << "," << filterjet_iter->bDiscriminator("combinedSecondaryVertexBJetTags") << std::endl;
        }
    ...
    for (edm::View < pat::Muon >::const_iterator mu = muons.begin();
        ...
        if (trkMu1Ref.isNonnull()) {
            ...
            const reco::TrackRef iTrack1 = mu->innerTrack();
        ...
        if (mu->isGlobalMuon()) {
            const reco::TrackRef gTrack = mu->globalTrack();
    ...
    /// Electrons
    edm::InputTag reducedEBRecHitCollection("reducedEcalRecHitsEB");
    edm::InputTag reducedEERecHitCollection("reducedEcalRecHitsEE");
    ...
    for (edm::View < pat::Electron >::const_iterator elec = electrons.begin();
        ...
        if (ele.gsfTrack().isNonnull()) {
            ...
            if (ele.gsfTrack().isNonnull()) {
                ...
                if (ip3dpv.first) {
                    ...
                    //ef.fMVAVar_ip3dSig = ip3d/ip3derr;
                }
            }
        }
        ...
        if (runOnMC_) {
            const reco::GenParticle *elecMc = elec->genLepton();
            if (elecMc != 0) {
    ...
    for (edm::View < pat::Tau >::const_iterator tau = taus.begin();
        ...
        if (tau->isTauIDAvailable("byLooseCombinedIsolationDeltaBetaCorr"))
            tf.byLooseCombinedIsolationDeltaBetaCorr = tau->tauID("byLooseCombinedIsolationDeltaBetaCorr");
        ...
        if (tau->isPFTau()) {
            ...
            if (tau->leadPFChargedHadrCand().isAvailable())
                tf.leadPFChargedHadrCandPt = tau->leadPFChargedHadrCand()->pt();
...
TVector2 HbbAnalyzerNew::getTvect(const pat::Jet * patJet) {
    ...
    for (size_t idx = 0; idx < patJetpfc.size(); idx++) {
        if (patJetpfc.at(idx)->charge() != 0) {
            ...
            r.Set(pi.Rapidity() - J.Rapidity(), reco::deltaPhi(patJetpfc.at(idx)->phi(), J.Phi()));
            r_mag = r.Mod();
...
void HbbAnalyzerNew::fillMET(VHbbEvent::METInfo & met,
    ...
    met.eLong = met_iter->e_longitudinal();
    met.p4 = GENPTOLORP(met_iter);
}
...
void HbbAnalyzerNew::fillSimpleJet(VHbbEvent::SimpleJet & sj,
    ...
    if (jet_iter->isPFJet()) {
        ...
        for (unsigned ic = 0; ic < constituents.size(); ++ic) {
            if (constituents[ic]->particleId() > 3)  // skip neutral constituents
                continue;
            const reco::TrackRef trackRef = constituents[ic]->trackRef();
