...
VHbbCandidate HbbCandidateFinderAlgo::changeHiggs(bool useHighestPtHiggs,
    ...
    TVector3 higgsBoost = temp.H.p4.BoostVector();
...
void HbbCandidateFinderAlgo::run(const VHbbEvent * event,
                                 ...
                                 bool isZbbHbb) {
    /// Search for leptons with final ID & isolation
    /// Assume event->muInfo, event->eleInfo are sorted
    ...
    /// Note: This is Type-1 corrected MET
    std::vector < VHbbEvent::METInfo > met;
    findMET(event->metType1, met);

    /// Check overlap between leptons and jets
    VHbbCandidateTools selector(verbose_);
    ...
    for (size_t j = 0; j < event->simpleJets2.size(); j++) {
        ...
        for (size_t i = 0; i < ele.size(); i++) {
            if (deltaR(ele[i].p4.Eta(), ele[i].p4.Phi(), event->simpleJets2[j].p4.Eta(), event->simpleJets2[j].p4.Phi()) < 0.5)
                overlap = true;
        }
    ...
    if (!foundJets && !foundHardJets)  return;
    ...
    /// See which kind of candidate this can be
    ...
    // ZbbHbb: all events are saved, regardless of whether V is found
    if (isZbbHbb) {
...
bool HbbCandidateFinderAlgo::findDiJets(const std::vector < VHbbEvent::SimpleJet > & jetsin,
    ...
    // No jetID
    unsigned int index1 = 9999, index2 = 9999;
    for (unsigned int i = 0; i < jets.size(); ++i) {
        if (jets[i].p4.Pt() > jetPtThreshold_ && fabs(jets[i].p4.Eta()) < etaThr /*&& jetID(jets[i])*/ ) {
            if (index1 == 9999) {
                index1 = i;
            } else if (index2 == 9999) {
                index2 = i;
...
bool HbbCandidateFinderAlgo::findFatJet(const std::vector < VHbbEvent::HardJet > & jetsin,
...
/*
    ...
    for(unsigned int kk=0;kk<subjets.size();kk++){
        ...
        subjets[kk].p4.Eta()  << " , " << subjets[kk].p4.Phi()  << std::endl;
    }
    for(unsigned int kk=0;kk<filterjets.size();kk++){
        std::cout << "filterjet  pt: " << filterjets[kk].p4.Pt() << " eta,phi " <<
        filterjets[kk].p4.Eta()  << " , " << filterjets[kk].p4.Phi()  << std::endl;
    }
*/
...
// debug
            //std::cout << "hardJet constituent pt: " << hardjets[i].subFourMomentum[j].Pt() << " eta,phi " << hardjets[i].subFourMomentum[j].Eta()  << " , " << hardjets[i].subFourMomentum[j].Phi()  << std::endl;
// debug
            //std::cout << "n: " << i << "," << j << " hardjetsub eta: " << roweta->GetPtr()[j] << " phi: " << rowphi->GetPtr()[j] << std::endl;
            for (unsigned int kk = 0; kk < filterjets.size(); kk++) {
                //if(subjets.eta[kk]==roweta->GetPtr()[j] && subjets.phi[kk]==rowphi->GetPtr()[j])
                //    subJetIn[j]=kk;
                //if(subjets[kk].p4.Eta()==hardjets[i].etaSub[j] && subjets[kk].p4.Phi()==hardjets[i].phiSub[j])
...
// debug
        ...
        for (int j = 0; j < 2; j++) {
            ...
            if (filterjets[subJetIn[j]].p4.Pt() > 20. && fabs(filterjets[subJetIn[j]].p4.Eta()) < etaThr && jetID(filterjets[subJetIn[j]]))
                nPt++;
        }
        ...
        for (unsigned int it = 0; it < muons.size(); ++it) {
            if (muons[it].globChi2<10 &&
                ...
                muons[it].p4.Pt()>20 ) {
                for (int j = 0; j < 2; j++) {
                    if (deltaR(muons[it].p4.Eta(), muons[it].p4.Phi(), filterjets[subJetIn[j]].p4.Eta(), filterjets[subJetIn[j]].p4.Phi()) < 0.3)
                        muOverlap++;
                }
            }
        }
...
//              !( fabs(electrons[it].p4.Eta()) < 1.57 && fabs(electrons[it].p4.Eta()) > 1.44) &&
                ...
                for (int j = 0; j < 2; j++) {
                    if (deltaR(electrons[it].p4.Eta(), electrons[it].p4.Phi(), filterjets[subJetIn[j]].p4.Eta(), filterjets[subJetIn[j]].p4.Phi()) < 0.3)
                        elecOverlap++;
                }
            }
        }
...
void HbbCandidateFinderAlgo::findMuons(const std::vector < VHbbEvent::MuonInfo > & muons,
...
void HbbCandidateFinderAlgo::findElectrons(const std::vector < VHbbEvent::ElectronInfo > & electrons,
...
void HbbCandidateFinderAlgo::findMET(const VHbbEvent::METInfo & met,
    ...
    out.push_back(met);
    if (verbose_) {
        std::cout << " CandidateFinder: Output MET = " << out.size() << std::endl;
    }
}


void HbbCandidateFinderAlgo::findTaus(const std::vector < VHbbEvent::TauInfo > & taus,
