...
bool HbbCandidateFinderAlgo::jetID(const VHbbEvent::SimpleJet & j) {
    if (j.neutralHadronEFraction > 0.99)
        return false;
    if (j.neutralEmEFraction > 0.99)
        return false;
...
VHbbCandidate HbbCandidateFinderAlgo::changeHiggs(bool useHighestPtHiggs,
                                                  const VHbbCandidate & old) {
    ...
    for (size_t i = 0; i < old.H.jets.size(); i++)
        jets.push_back(old.H.jets[i]);
    for (size_t i = 0; i < old.additionalJets.size(); i++)
...
void HbbCandidateFinderAlgo::run(const VHbbEvent * event,
...
//FIXME: cleaning here
    ...
    for (size_t j = 0; j < event->simpleJets2.size(); j++) {
        ...
        for (size_t i = 0; i < mu.size(); i++) {
            if (deltaR
                (mu[i].p4.Eta(), mu[i].p4.Phi(),
                 event->simpleJets2[j].p4.Eta(),
                 event->simpleJets2[j].p4.Phi()) < 0.5)
                overlap = true;
        }
        for (size_t i = 0; i < ele.size(); i++) {
            if (deltaR
                (ele[i].p4.Eta(), ele[i].p4.Phi(),
                 event->simpleJets2[j].p4.Eta(),
                 event->simpleJets2[j].p4.Phi()) < 0.5)
                overlap = true;
        }
    ...
    if (verbose_) {
        std::
            cout << " Found Dijets: " << foundJets << " Additional: " <<
            addJets.size() << std::endl;
    }
//  if (foundJets == false) return;
    ...
    foundHardJets =
        findFatJet(event->hardJets, event->subJets, event->filterJets, fatj1,
                   subJetsout, event->simpleJets2, addJetsFat, event->muInfo,
                   event->eleInfo);

    if (foundJets == false && foundHardJets == false)
        return;
    ...
    if (ele.size() < 1 && mu.size() < 1 && met.size() < 1 && tau.size() < 1)
        return;
    //
    // fill!
    //
    ...
    if (foundHardJets) {
        ...
        if (subJetsout.size() == 2) {
            ...
            temp.FatH.helicities.push_back(selector.
                                           getHelicity(subJetsout[0],
                                                       fathiggsBoost));
            temp.FatH.helicities.push_back(selector.
                                           getHelicity(subJetsout[1],
                                                       fathiggsBoost));
            temp.FatH.deltaTheta =
                selector.getDeltaTheta(subJetsout[0], subJetsout[1]);
        }
        if (subJetsout.size() == 3) {
            ...
            temp.FatH.p4 =
                subJetsout[0].p4 + subJetsout[1].p4 + subJetsout[2].p4;
            fathiggsBoost = (temp.FatH.p4).BoostVector();
            temp.FatH.helicities.push_back(selector.
                                           getHelicity(subJetsout[0],
                                                       fathiggsBoost));
    ...
    removeTauOverlapWithJets(tau, temp.H.jets, tauNoCandidateJetOverlap,
                             tauPos, tauPosNoCandidateJetOverlap);
...
void HbbCandidateFinderAlgo::findMET(const VHbbEvent::METInfo & met,
                                     std::vector < VHbbEvent::METInfo >
                                     &out) {
    //
    ...
    out.push_back(met);
    if (verbose_) {
        std::cout << " CandidateFinder: Input MET = " << met.
            metSig << " Output MET = " << out.size() << std::endl;
    }
...
bool HbbCandidateFinderAlgo::findDiJets(const std::vector <
                                        VHbbEvent::SimpleJet > &jetsin,
    ...
    if (verbose_) {
        std::cout << " CandidateFinder: Input Jets = " << jetsin.
            size() << std::endl;
    }
    ...
    for (unsigned int i = 0; i < jets.size(); ++i) {
        if (jets[i].p4.Pt() > jetPtThreshold_
            && fabs(jets[i].p4.Eta()) < etaThr /*&& jetID(jets[i]) */ ) {
            if (index1 == 999999) {
                index1 = i;
    ...
    if (index1 == 999999 || index2 == 999999)
        return false;
    ...
    if (verbose_) {
        std::
            cout << " CandidateFinder: Output Jets = " << 2 <<
            " Additional = " << addJets.size() << std::endl;
    }
...
bool HbbCandidateFinderAlgo::findDiJetsHighestPt(const std::vector <
                                                 VHbbEvent::SimpleJet >
                                                 &jetsin,
                                                 VHbbEvent::SimpleJet & j1,
                                                 VHbbEvent::SimpleJet & j2,
    ...
    for (unsigned int i = 0; i < jets.size() - 1; ++i) {
        for (unsigned int j = i + 1; j < jets.size(); ++j) {
            ...
            if (pt > highestPt && jets[j].p4.Pt() > jetPtThreshold_
                && jets[i].p4.Pt() > jetPtThreshold_
                && fabs(jets[i].p4.Eta()) < etaThr
                && fabs(jets[j].p4.Eta()) < etaThr && jetID(jets[i])
                && jetID(jets[j]) && jets[i].puJetIdL > 0
                && jets[j].puJetIdL > 0) {
                highestPt = pt;
                highesti = i;
                highestj = j;
    ...
    if (highesti == 999999 || highestj == 999999)
        return false;
    j1 = jets[highesti];
    j2 = jets[highestj];
...
bool HbbCandidateFinderAlgo::findFatJet(const std::vector <
                                        VHbbEvent::HardJet > &jetsin,
                                        const std::vector <
                                        VHbbEvent::SimpleJet > &subjetsin,
                                        const std::vector <
                                        VHbbEvent::SimpleJet > &filterjetsin,
                                        VHbbEvent::HardJet & fatj1,
                                        std::vector < VHbbEvent::SimpleJet >
                                        &subJetsout,
                                        const std::vector <
                                        VHbbEvent::SimpleJet > &ak5jetsin,
                                        std::vector < VHbbEvent::SimpleJet >
                                        &addJetsFat,
                                        const std::vector <
                                        VHbbEvent::MuonInfo > &muons,
                                        const std::vector <
                                        VHbbEvent::ElectronInfo >
                                        &electrons) {
...
//        subJetIn1st[In1st]=kk;}
                double deltaR_1 =
                    deltaR(subjets[kk].p4.Eta(), subjets[kk].p4.Phi(),
                           hardjets[i].subFourMomentum[j].Eta(),
                           hardjets[i].subFourMomentum[j].Phi());
                if (deltaR_1 < 0.01)
                    subJetIn1st[In1st] = kk;
            }
            In1st++;
...
//       if(subjets[kk].p4.Eta()==hardjets[i].subFourMomentum[j].Eta() && subjets[kk].p4.Phi()==hardjets[i].subFourMomentum[j].Phi()) subJetIn[j]=kk;
                double deltaR_1 =
                    deltaR(filterjets[kk].p4.Eta(), filterjets[kk].p4.Phi(),
                           hardjets[i].subFourMomentum[j].Eta(),
                           hardjets[i].subFourMomentum[j].Phi());
                if (deltaR_1 < 0.01)
                    subJetIn[j - 2] = kk;
...
// debug
        ...
        if (subJetIn1st[0] == -99 || subJetIn1st[1] == -99)
            continue;
        if (subJetIn[0] == -99 || subJetIn[1] == -99)
            continue;

        int nBtag = 0;
        for (int j = 0; j < 2; j++) {
            if (subjets[subJetIn1st[j]].csv > 0.)
                nBtag++;
        }
...
////     if(subjets[subJetIn1st[j]].p4.Pt()>30. && fabs(subjets[subJetIn1st[j]].p4.Eta())<etaThr && jetID(subjets[subJetIn1st[j]])) nPt++;}
            if (filterjets[subJetIn[j]].p4.Pt() > 20.
                && fabs(filterjets[subJetIn[j]].p4.Eta()) < etaThr
                && jetID(filterjets[subJetIn[j]]))
                nPt++;
        }
...
///// lepton overlap
        ...
        for (unsigned int it = 0; it < muons.size(); ++it) {
            if (muons[it].globChi2 < 10 &&
                ...
                muons[it].globNHits != 0 && muons[it].nHits > 10 &&
                //tracker
                (muons[it].cat & 0x1) &&
                //global
                (muons[it].cat & 0x2) &&
                muons[it].nMatches >= 2 &&
                muons[it].ipDb < .2 &&
                (muons[it].pfChaIso + muons[it].pfPhoIso +
                 muons[it].pfNeuIso) / muons[it].p4.Pt() < .15
                && fabs(muons[it].p4.Eta()) < 2.4 && muons[it].p4.Pt() > 20) {
                for (int j = 0; j < 2; j++) {
                    if (deltaR
                        (muons[it].p4.Eta(), muons[it].p4.Phi(),
                         filterjets[subJetIn[j]].p4.Eta(),
                         filterjets[subJetIn[j]].p4.Phi()) < 0.3)
                        muOverlap++;
                }
            }
        }
...
//      !( fabs(electrons[it].p4.Eta()) < 1.57 && fabs(electrons[it].p4.Eta()) > 1.44) &&
                   ...
                   && (electrons[it].pfChaIso + electrons[it].pfPhoIso +
                       electrons[it].pfNeuIso) / electrons[it].p4.Pt() <
                   .15) {
                for (int j = 0; j < 2; j++) {
                    if (deltaR
                        (electrons[it].p4.Eta(), electrons[it].p4.Phi(),
                         filterjets[subJetIn[j]].p4.Eta(),
                         filterjets[subJetIn[j]].p4.Phi()) < 0.3)
                        elecOverlap++;
                }
...
//      minBtag1=subjets[subJetIn1st[0]].csv+subjets[subJetIn1st[1]].csv;       
        if ((subjets[subJetIn1st[0]].p4 + subjets[subJetIn1st[1]].p4).Pt() >
            minBtag1) {
            minBtag1 =
                (subjets[subJetIn1st[0]].p4 +
                 subjets[subJetIn1st[1]].p4).Pt();
/*       double filtpt=0;
...
//       minBtag1=hardjets[i].p4.Pt();  
    ...
    for (unsigned int i = 0; i < ak5jets.size(); ++i) {
        ...
        for (unsigned int j = 0; j < subJetsout.size(); ++j) {
            if (subJetsout[j].p4.Pt() < 20.)
                continue;
            if (deltaR
                (ak5jets[i].p4.Eta(), ak5jets[i].p4.Phi(),
                 subJetsout[j].p4.Eta(), subJetsout[j].p4.Phi()) < 0.3)
                overlap++;
        }
        if (overlap == 0)
            addJetsFat.push_back(ak5jets[i]);
    }
...
void HbbCandidateFinderAlgo::removeTauOverlapWithJets(const std::vector <
                                                      VHbbEvent::TauInfo >
                                                      &taus,
                                                      const std::vector <
                                                      VHbbEvent::SimpleJet >
    ...
    for (unsigned int it = 0; it < taus.size(); ++it) {
        ...
        for (unsigned int jit = 0; jit < jets.size(); ++jit) {
            if (taus[it].p4.DeltaR(jets[jit].p4) < 0.2) {
                ...
                if (verbose_) {
                    std::
                        cout << "Found overlap of tau (pt,eta,phi)=(" <<
                        taus[it].p4.Pt() << "," << taus[it].p4.
                        Eta() << "," << taus[it].p4.
                        Phi() << ")" << "with candidate jet (pt,eta,phi)=(" <<
                        jets[jit].p4.Pt() << "," << jets[jit].p4.
                        Eta() << "," << jets[jit].p4.
                        Phi() << ")" << std::endl;
                }
            }
        ...
        if (!overlap) {
            if (verbose_) {
                std::cout << "No overlap with tau (pt,eta,phi)=(" << taus[it].
                    p4.Pt() << "," << taus[it].p4.Eta() << "," << taus[it].p4.
                    Phi() << "); keeping it " << std::endl;
            }
...
void HbbCandidateFinderAlgo::findTaus(const std::vector < VHbbEvent::TauInfo >
                                      &taus,
                                      std::vector < VHbbEvent::TauInfo > &out,
                                      std::vector <
                                      unsigned int >&positions) {
    if (verbose_)
        std::cout << "[SCZ] Tau size=" << taus.size() << std::endl;
    for (unsigned int it = 0; it < taus.size(); ++it) {
        /*
        ...
        if (verbose_) {
            std::
                cout <<
                "(pt,decayModeFinding,byLooseCombinedIsolationDeltaBetaCorr,againstMuonTight,againstElectronLoose,againstElectronMedium,againstElectronMVA)=("
                << taus[it].p4.Pt() << "," << (taus[it].decayModeFinding >
                                               0.5) << "," << (taus[it].
                                                               byLooseCombinedIsolationDeltaBetaCorr
                                                               >
                                                               0.5) << "," <<
                (taus[it].againstMuonTight >
                 0.5) << "," << (taus[it].againstElectronLoose >
                                 0.5) << "," << (taus[it].
        ...
        if (taus[it].decayModeFinding > 0.5
    ...
    if (verbose_) {
        std::cout << " CandidateFinder: Input Taus = " << taus.
            size() << " Output Taus = " << out.size() << std::endl;
    }
}


void HbbCandidateFinderAlgo::findMuons(const std::vector <
                                       VHbbEvent::MuonInfo > &muons,
                                       std::vector < VHbbEvent::MuonInfo >
                                       &out,
...
*/
    ...
    for (unsigned int it = 0; it < muons.size(); ++it) {
        ...
        float pfCorrIso =
            (muons[it].pfChaIso +
             std::max(muons[it].pfPhoIso + muons[it].pfNeuIso - rhoN * area,
                      mincor)) / muons[it].p4.Pt();
        if (muons[it].isPF && muons[it].globChi2 < 10
            && muons[it].nPixelHits >= 1 && muons[it].globNHits != 0
...
//      (muons[it].cat & 0x2) && 
            ...
            (muons[it].cat & 0x1) &&
            muons[it].nMatches >= 2 &&
            muons[it].ipDb < .2 && muons[it].zPVPt < 0.5 &&
    ...
    if (verbose_) {
        std::cout << " CandidateFinder: Input Muons = " << muons.
            size() << " Output Muons = " << out.size() << std::endl;
    }
...
void HbbCandidateFinderAlgo::findElectrons(const std::vector <
                                           VHbbEvent::ElectronInfo >
                                           &electrons,
                                           std::vector <
                                           VHbbEvent::ElectronInfo > &out,
                                           std::vector <
                                           unsigned int >&positions,
                                           const VHbbEventAuxInfo & aux) {
    /*
...
//Correct electron photon double count
        ...
        float pfCorrIso =
            (electrons[it].pfChaIso +
             std::max(pho - rho * areagamma,
                      mincor) + std::max(electrons[it].pfNeuIso -
                                         rho * areaNH,
                                         mincor)) / electrons[it].p4.Pt();
        float iso = pfCorrIso;
        float id = electrons[it].mvaOutTrig;
        bool wp70 = ((fabs(eta) < 0.8 && id > 0.977 && iso < 0.093)
                     || (fabs(eta) >= 0.8 && fabs(eta) < 1.479 && id > 0.956
                         && iso < 0.095) || (fabs(eta) >= 1.479
                                             && fabs(eta) < 2.5 && id > 0.966
                                             && iso < 0.171));
        bool wp80 = ((fabs(eta) < 0.8 && id > 0.913 && iso < 0.105)
                     || (fabs(eta) >= 0.8 && fabs(eta) < 1.479 && id > 0.964
                         && iso < 0.178) || (fabs(eta) >= 1.479
                                             && fabs(eta) < 2.5 && id > 0.899
                                             && iso < 0.150));
        bool wp85 = ((fabs(eta) < 0.8 && id > 0.929 && iso < 0.135)
                     || (fabs(eta) >= 0.8 && fabs(eta) < 1.479 && id > 0.931
                         && iso < 0.159) || (fabs(eta) >= 1.479
                                             && fabs(eta) < 2.5 && id > 0.805
                                             && iso < 0.155));
        bool wp90 = ((fabs(eta) < 0.8 && id > 0.877 && iso < 0.177)
                     || (fabs(eta) >= 0.8 && fabs(eta) < 1.479 && id > 0.794
                         && iso < 0.180) || (fabs(eta) >= 1.479
                                             && fabs(eta) < 2.5 && id > 0.846
                                             && iso < 0.244));
        bool wp95 = ((fabs(eta) < 0.8 && id > 0.858 && iso < 0.253)
                     || (fabs(eta) >= 0.8 && fabs(eta) < 1.479 && id > 0.425
                         && iso < 0.225) || (fabs(eta) >= 1.479
                                             && fabs(eta) < 2.5 && id > 0.759
                                             && iso < 0.308));
        bool wpHWW = ((fabs(eta) < 0.8 && id > 0.94 && iso < 0.15)
                      || (fabs(eta) >= 0.8 && fabs(eta) < 1.479 && id > 0.85
                          && iso < 0.15) || (fabs(eta) >= 1.479
                                             && fabs(eta) < 2.5 && id > 0.92
                                             && iso < 0.15));
...
//      (fabs(electrons[it].id95 - 7)) < 0.1  &&
               wp95 && ((electrons[it].isEE &&
                        ...
                        || (electrons[it].isEB &&
                            ...
                            electrons[it].innerHits == 0 &&
                            (electrons[it].tIso / electrons[it].p4.Pt()) < 0.2
                            && (electrons[it].eIso / electrons[it].p4.Pt()) <
                            0.2
                            && (electrons[it].hIso / electrons[it].p4.Pt()) <
                            0.2)
               ) &&
...
*/
    ...
    if (verbose_) {
        std::cout << " CandidateFinder: Input Electrons = " << electrons.
            size() << " Output Electrons = " << out.size() << std::endl;
    }
