...
template <> void LeptonInfo::setSpecific < VHbbEvent::ElectronInfo >
    ...
    id80[j] = i.id80;
    id95[j] = i.id95;
    id80NoIso[j] = (i.innerHits == 0
                    && !(fabs(i.convDist) < 0.02 && fabs(i.convDcot) < 0.02)
                    &&
                    ((i.isEB && i.sihih < 0.01 && fabs(i.Dphi) < 0.06
                      && fabs(i.Deta) < 0.004) || (i.isEE && i.sihih < 0.03
                                                   && fabs(i.Dphi) < 0.03
                                                   && fabs(i.Deta) < 0.007)));
...
*/
    ...
    pfCorrIso[j] =
        (i.pfChaIso + std::max(pho - rho * areagamma, mincor) +
         std::max(i.pfNeuIso - rho * areaNH, mincor)) / i.p4.Pt();

    id2012tight[j] = fabs(i.dxy) < 0.02 && fabs(i.dz) < 0.1 && ((i.isEE
                                                                 && fabs(i.
                                                                         Deta)
                                                                 < 0.005
                                                                 && fabs(i.
    ...
    bool mvaPreSel = ((i.isEE &&
                       ...
                       i.innerHits == 0 &&
                       i.tIso / i.p4.Pt() < 0.2 &&
                       i.eIso / i.p4.Pt() < 0.2 && i.hIso / i.p4.Pt() < 0.2)
                      || (i.isEB &&
    ...
    wp70[j] = ((fabs(eta) < 0.8 && id > 0.977 && iso < 0.093)
               || (fabs(eta) >= 0.8 && fabs(eta) < 1.479 && id > 0.956
                   && iso < 0.095) || (fabs(eta) >= 1.479 && fabs(eta) < 2.5
                                       && id > 0.966 && iso < 0.171));
    wp80[j] = ((fabs(eta) < 0.8 && id > 0.913 && iso < 0.105)
               || (fabs(eta) >= 0.8 && fabs(eta) < 1.479 && id > 0.964
                   && iso < 0.178) || (fabs(eta) >= 1.479 && fabs(eta) < 2.5
                                       && id > 0.899 && iso < 0.150));
    wp85[j] = ((fabs(eta) < 0.8 && id > 0.929 && iso < 0.135)
               || (fabs(eta) >= 0.8 && fabs(eta) < 1.479 && id > 0.931
                   && iso < 0.159) || (fabs(eta) >= 1.479 && fabs(eta) < 2.5
                                       && id > 0.805 && iso < 0.155));
    wp90[j] = ((fabs(eta) < 0.8 && id > 0.877 && iso < 0.177)
               || (fabs(eta) >= 0.8 && fabs(eta) < 1.479 && id > 0.794
                   && iso < 0.180) || (fabs(eta) >= 1.479 && fabs(eta) < 2.5
                                       && id > 0.846 && iso < 0.244));
    wp95[j] = ((fabs(eta) < 0.8 && id > 0.858 && iso < 0.253)
               || (fabs(eta) >= 0.8 && fabs(eta) < 1.479 && id > 0.425
                   && iso < 0.225) || (fabs(eta) >= 1.479 && fabs(eta) < 2.5
                                       && id > 0.759 && iso < 0.308));
    wpHWW[j] = ((fabs(eta) < 0.8 && id > 0.94 && iso < 0.15)
                || (fabs(eta) >= 0.8 && fabs(eta) < 1.479 && id > 0.85
                    && iso < 0.15) || (fabs(eta) >= 1.479 && fabs(eta) < 2.5
                                       && id > 0.92 && iso < 0.15));
...
template <> void LeptonInfo::setSpecific < VHbbEvent::MuonInfo >
    ...
    dxy[j] = i.ipDb;
    dz[j] = i.zPVPt;
    vbtf[j] = (i.globChi2 < 10 && i.nPixelHits >= 1 && i.globNHits != 0
               && i.nHits > 10 && i.cat & 0x1 && i.cat & 0x2
               && i.nMatches >= 2 && i.ipDb < .2
               && (i.pfChaIso + i.pfPhoIso + i.pfNeuIso) / i.p4.Pt() < .15
    ...
    pfCorrIso[j] =
        (i.pfChaIso +
         std::max(i.pfPhoIso + i.pfNeuIso - rhoN * area, mincor)) / i.p4.Pt();
    id2012tight[j] = i.isPF && i.globChi2 < 10 && i.nPixelHits >= 1
        && i.globNHits != 0 && i.nValidLayers > 5 && (i.cat & 0x2)
        && i.nMatches >= 2 && i.ipDb < .2;
...
typedef struct {
    void set(const VHbbEvent::SimpleJet & j, int i) {
        ...
        {
            csv_nominal[i] =
                nominalShape->reshape(eta[i], pt[i], j.csv, j.flavour);
            csv_upBC[i] = upBCShape->reshape(eta[i], pt[i], j.csv, j.flavour);
            csv_downBC[i] =
                downBCShape->reshape(eta[i], pt[i], j.csv, j.flavour);
            csv_upL[i] = upLShape->reshape(eta[i], pt[i], j.csv, j.flavour);
            csv_downL[i] =
                downLShape->reshape(eta[i], pt[i], j.csv, j.flavour);
            csv_nominal4p[i] =
                nominalShape4p->reshape(eta[i], pt[i], j.csv, j.flavour);
            csv_upBC4p[i] =
                upBCShape4p->reshape(eta[i], pt[i], j.csv, j.flavour);
            csv_downBC4p[i] =
                downBCShape4p->reshape(eta[i], pt[i], j.csv, j.flavour);
        } else
        {
            csv_nominal[i] = csv[i];
            csv_downBC[i] = csv[i];
    ...
    void reset() {
        for (int i = 0; i < MAXJ; i++) {
            ...
            nconstituents[i] = -99;
            flavour[i] = -99;
            isSemiLeptMCtruth[i] = -99;
            isSemiLept[i] = -99;
            SoftLeptpdgId[i] = -99;
            SoftLeptIdlooseMu[i] = -99;
            SoftLeptId95[i] = -99;
            SoftLeptPt[i] = -99;
            SoftLeptdR[i] = -99;
            SoftLeptptRel[i] = -99;
            SoftLeptRelCombIso[i] = -99;
            genPt[i] = -99;
            genEta[i] = -99;
            genPhi[i] = -99;
            JECUnc[i] = -99;
...
int main(int argc, char *argv[]) {
    ...
    float HVdPhi, HVMass, HMETdPhi, VMt, deltaPullAngle, deltaPullAngleAK7,
        ...
        genHpt, weightTrig, weightTrigMay, weightTrigV4, weightTrigMET,
        weightTrigOrMu30, minDeltaPhijetMET, jetPt_minDeltaPhijetMET,
        PUweight, PUweightP, PUweightM, PUweightAB, PUweight2011B,
        PUweight1DObs;
    float PU0, PUp1, PUm1;
    ...
    bool triggerFlags[500], hbhe, ecalFlag, totalKinematics, cschaloFlag,
        hcallaserFlag, trackingfailureFlag, eebadscFlag;


    float btag1T2CSF = 1., btag2TSF = 1., btag1TSF = 1., btagA0CSF =
        1., btagA0TSF = 1., btag2CSF = 1., btag1TA1C = 1.;
    // ----------------------------------------------------------------------
    // First Part: 
    //
    ...
    std::vector < VHbbCandidate > *candZlocal =
        new std::vector < VHbbCandidate >;
    std::vector < VHbbCandidate > *candWlocal =
        new std::vector < VHbbCandidate >;

    // get the python configuration
    PythonProcessDesc builder(argv[1]);
    const edm::ParameterSet & in =
        builder.processDesc()->getProcessPSet()->getParameter <
        edm::ParameterSet > ("fwliteInput");
    const edm::ParameterSet & out =
        builder.processDesc()->getProcessPSet()->getParameter <
        edm::ParameterSet > ("fwliteOutput");
    const edm::ParameterSet & ana =
        builder.processDesc()->getProcessPSet()->getParameter <
        edm::ParameterSet > ("Analyzer");
    ...
    if (in.exists("lumisToProcess")) {
        std::vector < edm::LuminosityBlockRange > const &lumisTemp =
            in.getUntrackedParameter < std::vector <
            edm::LuminosityBlockRange > >("lumisToProcess");
    ...
    unsigned int outputEvery_(in.getParameter <
                              unsigned int >("outputEvery"));
    std::string outputFile_(out.getParameter < std::string > ("fileName"));
    std::vector < std::string > triggers(ana.getParameter < std::vector <
                                         std::string > >("triggers"));
    double btagThr = ana.getParameter < double >("bJetCountThreshold");
    bool fromCandidate = ana.getParameter < bool > ("readFromCandidates");
    bool useHighestPtHiggsZ =
        ana.getParameter < bool > ("useHighestPtHiggsZ");
    bool useHighestPtHiggsW =
        ana.getParameter < bool > ("useHighestPtHiggsW");
    HbbCandidateFinderAlgo *algoZ =
        new HbbCandidateFinderAlgo(ana.getParameter < bool > ("verbose"),
                                   ana.getParameter <
                                   double >("jetPtThresholdZ"),
                                   useHighestPtHiggsZ);
    HbbCandidateFinderAlgo *algoW =
        new HbbCandidateFinderAlgo(ana.getParameter < bool > ("verbose"),
                                   ana.getParameter <
                                   double >("jetPtThresholdW"),
                                   useHighestPtHiggsW);
    HbbCandidateFinderAlgo *algoRecoverLowPt =
        new HbbCandidateFinderAlgo(ana.getParameter < bool > ("verbose"), 15,
                                   true);

    TriggerWeight triggerWeight(ana);

    const edm::ParameterSet & anaAB =
        builder.processDesc()->getProcessPSet()->getParameter <
        edm::ParameterSet > ("Analyzer2012ABOnly");
    TriggerWeight triggerWeightAB(anaAB);

    BTagWeight btag(2);         // 2 operating points "Custom" = 0.5 and "Tight = 0.898"
    BTagSampleEfficiency btagEff(ana.getParameter < std::string >
                                 ("btagEffFileName").c_str());

    std::vector < std::string > inputFiles_(in.getParameter < std::vector <
                                            std::string > >("fileNames"));
    //  std::string inputFile( in.getParameter<std::string> ("fileName") );

    std::string PUmcfileName_ =
        in.getParameter < std::string > ("PUmcfileName");
    std::string PUmcfileName2011B_ =
        in.getParameter < std::string > ("PUmcfileName2011B");

    std::string PUdatafileNameAB =
        in.getParameter < std::string > ("PUdatafileNameAB");
    std::string PUdatafileName_ =
        in.getParameter < std::string > ("PUdatafileName");
    std::string PUdatafileName_plusOneSigma =
        in.getParameter < std::string > ("PUdatafileNamePlus");
    std::string PUdatafileName_minusOneSigma =
        in.getParameter < std::string > ("PUdatafileNameMinus");
    std::string PUdatafileName2011B_ =
        in.getParameter < std::string > ("PUdatafileName2011B");
    std::string Weight3DfileName_ =
        in.getParameter < std::string > ("Weight3DfileName");
    ...
    if (isMC_) {
        nominalShape =
            new BTagShapeInterface(ana.getParameter < std::string >
                                   ("csvDiscr").c_str(), 0.0, 0.0);
        upBCShape =
            new BTagShapeInterface(ana.getParameter < std::string >
                                   ("csvDiscr").c_str(), 1.5, 0.0);
        downBCShape =
            new BTagShapeInterface(ana.getParameter < std::string >
                                   ("csvDiscr").c_str(), -1.5, 0.0);
        upLShape =
            new BTagShapeInterface(ana.getParameter < std::string >
                                   ("csvDiscr").c_str(), 0.0, 1.0);
        downLShape =
            new BTagShapeInterface(ana.getParameter < std::string >
                                   ("csvDiscr").c_str(), 0.0, -1.0);
        nominalShape4p =
            new BTagShapeInterface(ana.getParameter < std::string >
                                   ("csvDiscr").c_str(), 0.0, 0.0, true,
                                   1.003, 1.003);
        upBCShape4p =
            new BTagShapeInterface(ana.getParameter < std::string >
                                   ("csvDiscr").c_str(), 0.0, 0.0, true,
...
// edm::Lumi3DReWeighting   lumiWeights2011B;
    if (isMC_) {
        lumiWeights =
            edm::LumiReWeighting(PUmcfileName_, PUdatafileName_, "pileup",
                                 "pileup");
        lumiWeightsPl =
            edm::LumiReWeighting(PUmcfileName_, PUdatafileName_plusOneSigma,
                                 "pileup", "pileup");
        lumiWeightsMi =
            edm::LumiReWeighting(PUmcfileName_, PUdatafileName_minusOneSigma,
                                 "pileup", "pileup");
        lumiWeightsAB =
            edm::LumiReWeighting(PUmcfileName_, PUdatafileNameAB, "pileup",
                                 "pileup");
        lumiWeights1DObs =
            edm::LumiReWeighting(PUmcfileName2011B_, PUdatafileName2011B_,
                                 "pileup", "pileup");
...
//                    }
    ...
    TH1F *countWithPUP =
        new TH1F("CountWithPUP", "CountWithPU plus one sigma", 1, 0, 2);
    TH1F *countWithPUM =
        new TH1F("CountWithPUM", "CountWithPU minus one sigma", 1, 0, 2);
    TH1F *countWithPUAB =
        new TH1F("CountWithPUAB", "CountWithPU 2012AB", 1, 0, 2);

    TH1F *countWithPU2011B =
        new TH1F("CountWithPU2011B", "CountWithPU2011B", 1, 0, 2);
    TH3F *input3DPU =
        new TH3F("Input3DPU", "Input3DPU", 36, -0.5, 35.5, 36, -0.5, 35.5, 36,
                 -0.5, 35.5);
    ...
    _outTree->Branch("H", &H,
                     "HiggsFlag/I:mass/F:pt/F:eta:phi/F:dR/F:dPhi/F:dEta/F");
    _outTree->Branch("V", &V, "mass/F:pt/F:eta:phi/F");
    _outTree->Branch("FatH", &FatH,
                     "FatHiggsFlag/I:mass/F:pt/F:eta:phi/F:filteredmass/F:filteredpt/F:filteredeta/F:filteredphi/F");
    _outTree->Branch("lheV_pt", &lheV_pt, "lheV_pt/F");
    _outTree->Branch("lheHT", &lheHT, "lheHT/F");
    _outTree->Branch("lheNj", &lheNj, "lheNj/F");
    _outTree->Branch("genZ", &genZ,
                     "mass/F:pt/F:eta:phi/F:status/F:charge:momid/F");
    _outTree->Branch("genZstar", &genZstar,
                     "mass/F:pt/F:eta:phi/F:status/F:charge:momid/F");
    _outTree->Branch("genW", &genW,
                     "mass/F:pt/F:eta:phi/F:status/F:charge:momid/F");
    _outTree->Branch("genWstar", &genWstar,
                     "mass/F:pt/F:eta:phi/F:status/F:charge:momid/F");
    _outTree->Branch("genH", &genH,
                     "mass/F:pt/F:eta:phi/F:status/F:charge:momid/F");
    _outTree->Branch("genB", &genB,
                     "mass/F:pt/F:eta:phi/F:status/F:charge:momid/F");
    _outTree->Branch("genBbar", &genBbar,
                     "mass/F:pt/F:eta:phi/F:status/F:charge:momid/F");

    _outTree->Branch("genTop", &genTop,
                     "bmass/F:bpt/F:beta:bphi/F:bstatus/F:wdau1mass/F:wdau1pt/F:wdau1eta:wdau1phi/F:wdau1id/F:wdau2mass/F:wdau2pt/F:wdau2eta:wdau2phi/F:wdau2id/F");
    _outTree->Branch("genTbar", &genTbar,
                     "bmass/F:bpt/F:beta:bphi/F:bstatus/F:wdau1mass/F:wdau1pt/F:wdau1eta:wdau1phi/F:wdau1id/F:wdau2mass/F:wdau2pt/F:wdau2eta:wdau2phi/F:wdau2id/F");

    _outTree->Branch("TkSharing", &TkSharing,
                     "HiggsCSVtkSharing/b:HiggsIPtkSharing:HiggsSVtkSharing:FatHiggsCSVtkSharing:FatHiggsIPtkSharing:FatHiggsSVtkSharing");

    _outTree->Branch("nhJets", &nhJets, "nhJets/I");
    _outTree->Branch("nfathFilterJets", &nfathFilterJets,
                     "nfathFilterJets/I");
    ...
    _outTree->Branch("hJet_csv_nominal", hJets.csv_nominal,
                     "csv_nominal[nhJets]/F");
    _outTree->Branch("hJet_csv_upBC", hJets.csv_upBC, "csv_upBC[nhJets]/F");
    _outTree->Branch("hJet_csv_downBC", hJets.csv_downBC,
                     "csv_downBC[nhJets]/F");
    _outTree->Branch("hJet_csv_upL", hJets.csv_upL, "csv_upL[nhJets]/F");
    _outTree->Branch("hJet_csv_downL", hJets.csv_downL,
                     "csv_downL[nhJets]/F");
    _outTree->Branch("hJet_csv_nominal4p", hJets.csv_nominal4p,
                     "csv_nominal4p[nhJets]/F");
    _outTree->Branch("hJet_csv_upBC4p", hJets.csv_upBC4p,
                     "csv_upBC4p[nhJets]/F");
    _outTree->Branch("hJet_csv_downBC4p", hJets.csv_downBC4p,
                     "csv_downBC4p[nhJets]/F");
    ...
    _outTree->Branch("hJet_numTracksSV", hJets.numTracksSV,
                     "numTracksSV[nhJets]/I");
    ...
    _outTree->Branch("hJet_nconstituents", hJets.nconstituents,
                     "nconstituents[nhJets]");
    _outTree->Branch("hJet_flavour", hJets.flavour, "flavour[nhJets]/F");
    _outTree->Branch("hJet_isSemiLept", hJets.isSemiLept,
                     "isSemiLept[nhJets]/I");
    _outTree->Branch("hJet_isSemiLeptMCtruth", hJets.isSemiLeptMCtruth,
                     "isSemiLeptMCtruth[nhJets]/I");
    _outTree->Branch("hJet_SoftLeptpdgId", hJets.SoftLeptpdgId,
                     "SoftLeptpdgId[nhJets]/I");
    _outTree->Branch("hJet_SoftLeptIdlooseMu", hJets.SoftLeptIdlooseMu,
                     "SoftLeptIdlooseMu[nhJets]/I");
    _outTree->Branch("hJet_SoftLeptId95", hJets.SoftLeptId95,
                     "SoftLeptId95[nhJets]/I");
    _outTree->Branch("hJet_SoftLeptPt", hJets.SoftLeptPt,
                     "SoftLeptPt[nhJets]/F");
    _outTree->Branch("hJet_SoftLeptdR", hJets.SoftLeptdR,
                     "SoftLeptdR[nhJets]/F");
    _outTree->Branch("hJet_SoftLeptptRel", hJets.SoftLeptptRel,
                     "SoftLeptptRel[nhJets]/F");
    _outTree->Branch("hJet_SoftLeptRelCombIso", hJets.SoftLeptRelCombIso,
                     "SoftLeptRelCombIso[nhJets]/F");
    ...
    _outTree->Branch("hJet_SF_CSVLerr", hJets.SF_CSVLerr,
                     "SF_CSVLerr[nhJets]/b");
    _outTree->Branch("hJet_SF_CSVMerr", hJets.SF_CSVMerr,
                     "SF_CSVMerr[nhJets]/b");
    _outTree->Branch("hJet_SF_CSVTerr", hJets.SF_CSVTerr,
                     "SF_CSVTerr[nhJets]/b");
    _outTree->Branch("hJet_ptRaw", hJets.ptRaw, "ptRaw[nhJets]/F");
    _outTree->Branch("hJet_ptLeadTrack", hJets.ptLeadTrack,
                     "ptLeadTrack[nhJets]/F");
    ...
    _outTree->Branch("hJet_puJetIdMva", hJets.puJetIdMva,
                     "puJetIdMva[nhJets]/F");
    _outTree->Branch("hJet_charge", hJets.charge, "charge[nhJets]/F");

    _outTree->Branch("fathFilterJets_pt", fathFilterJets.pt,
                     "pt[nfathFilterJets]/F");
    _outTree->Branch("fathFilterJets_eta", fathFilterJets.eta,
                     "eta[nfathFilterJets]/F");
    _outTree->Branch("fathFilterJets_phi", fathFilterJets.phi,
                     "phi[nfathFilterJets]/F");
    _outTree->Branch("fathFilterJets_e", fathFilterJets.e,
                     "e[nfathFilterJets]/F");
    _outTree->Branch("fathFilterJets_csv", fathFilterJets.csv,
                     "csv[nfathFilterJets]/F");
    _outTree->Branch("fathFilterJets_chf", fathFilterJets.chf,
                     "chf[nfathFilterJets]/F");
    _outTree->Branch("fathFilterJets_ptRaw", fathFilterJets.ptRaw,
                     "ptRaw[nfathFilterJets]/F");
    _outTree->Branch("fathFilterJets_ptLeadTrack", fathFilterJets.ptLeadTrack,
                     "ptLeadTrack[nfathFilterJets]/F");
    _outTree->Branch("fathFilterJets_flavour", fathFilterJets.flavour,
                     "flavour[nfathFilterJets]/F");
    _outTree->Branch("fathFilterJets_isSemiLept", fathFilterJets.isSemiLept,
                     "isSemiLept[nfathFilterJets]/F");
    _outTree->Branch("fathFilterJets_isSemiLeptMCtruth",
                     fathFilterJets.isSemiLeptMCtruth,
                     "isSemiLeptMCtruth[nfathFilterJets]/F");
    _outTree->Branch("fathFilterJets_genPt", fathFilterJets.genPt,
                     "genPt[nfathFilterJets]/F");
    _outTree->Branch("fathFilterJets_genEta", fathFilterJets.genEta,
                     "genEta[nfathFilterJets]/F");
    _outTree->Branch("fathFilterJets_genPhi", fathFilterJets.genPhi,
                     "genPhi[nfathFilterJets]/F");
    _outTree->Branch("fathFilterJets_vtxMass", fathFilterJets.vtxMass,
                     "vtxMass[nfathFilterJets]/F");
    _outTree->Branch("fathFilterJets_vtx3dL", fathFilterJets.vtx3dL,
                     "vtx3dL[nfathFilterJets]/F");
    _outTree->Branch("fathFilterJets_vtx3deL", fathFilterJets.vtx3deL,
                     "vtx3deL[nfathFilterJets]/F");
    _outTree->Branch("fathFilterJets_vtxPt", fathFilterJets.vtxPt,
                     "vtxPt[nfathFilterJets]/F");
    _outTree->Branch("fathFilterJets_vtxEta", fathFilterJets.vtxEta,
                     "vtxEta[nfathFilterJets]/F");
    _outTree->Branch("fathFilterJets_vtxPhi", fathFilterJets.vtxPhi,
                     "vtxPhi[nfathFilterJets]/F");
    _outTree->Branch("fathFilterJets_vtxE", fathFilterJets.vtxE,
                     "vtxE[nfathFilterJets]/F");
...
//  _outTree->Branch("fathFilterJets_csv",fathFilterJets.csv ,"csv[nfathFilterJets]/F");
    _outTree->Branch("fathFilterJets_csvivf", fathFilterJets.csvivf,
                     "csvivf[nfathFilterJets]/F");
    _outTree->Branch("fathFilterJets_cmva", fathFilterJets.cmva,
                     "cmva[nfathFilterJets]/F");
//  _outTree->Branch("fathFilterJets_flavour",fathFilterJets.flavour ,"flavour[nfathFilterJets]/F");
    _outTree->Branch("fathFilterJets_cosTheta", fathFilterJets.cosTheta,
                     "cosTheta[nfathFilterJets]/F");
    _outTree->Branch("fathFilterJets_jetArea", fathFilterJets.jetArea,
                     "jetArea[nfathFilterJets]/F");
    ...
    _outTree->Branch("aJet_csv_nominal", aJets.csv_nominal,
                     "csv_nominal[naJets]/F");
    _outTree->Branch("aJet_csv_upBC", aJets.csv_upBC, "csv_upBC[naJets]/F");
    _outTree->Branch("aJet_csv_downBC", aJets.csv_downBC,
                     "csv_downBC[naJets]/F");
    _outTree->Branch("aJet_csv_upL", aJets.csv_upL, "csv_upL[naJets]/F");
    _outTree->Branch("aJet_csv_downL", aJets.csv_downL,
                     "csv_downL[naJets]/F");
    _outTree->Branch("aJet_csv_nominal4p", aJets.csv_nominal4p,
                     "csv_nominal4p[naJets]/F");
    _outTree->Branch("aJet_csv_upBC4p", aJets.csv_upBC4p,
                     "csv_upBC4p[naJets]/F");
    _outTree->Branch("aJet_csv_downBC4p", aJets.csv_downBC4p,
                     "csv_downBC4p[naJets]/F");
    ...
    _outTree->Branch("aJet_numTracksSV", aJets.numTracksSV,
                     "numTracksSV[naJets]/I");
    ...
    _outTree->Branch("aJet_nconstituents", aJets.nconstituents,
                     "nconstituents[naJets]");
    _outTree->Branch("aJet_flavour", aJets.flavour, "flavour[naJets]/F");
    _outTree->Branch("aJet_isSemiLept", aJets.isSemiLept,
                     "isSemiLept[naJets]/I");
    _outTree->Branch("aJet_isSemiLeptMCtruth", aJets.isSemiLeptMCtruth,
                     "isSemiLeptMCtruth[naJets]/I");
    _outTree->Branch("aJet_SoftLeptpdgId", aJets.SoftLeptpdgId,
                     "SoftLeptpdgId[naJets]/I");
    _outTree->Branch("aJet_SoftLeptIdlooseMu", aJets.SoftLeptIdlooseMu,
                     "SoftLeptIdlooseMu[naJets]/I");
    _outTree->Branch("aJet_SoftLeptId95", aJets.SoftLeptId95,
                     "SoftLeptId95[naJets]/I");
    _outTree->Branch("aJet_SoftLeptPt", aJets.SoftLeptPt,
                     "SoftLeptPt[naJets]/F");
    _outTree->Branch("aJet_SoftLeptdR", aJets.SoftLeptdR,
                     "SoftLeptdR[naJets]/F");
    _outTree->Branch("aJet_SoftLeptptRel", aJets.SoftLeptptRel,
                     "SoftLeptptRel[naJets]/F");
    _outTree->Branch("aJet_SoftLeptRelCombIso", aJets.SoftLeptRelCombIso,
                     "SoftLeptRelCombIso[naJets]/F");
    ...
    _outTree->Branch("aJet_puJetIdMva", aJets.puJetIdMva,
                     "puJetIdMva[naJets]/F");
    ...
    _outTree->Branch("aJet_SF_CSVLerr", aJets.SF_CSVLerr,
                     "SF_CSVLerr[naJets]/b");
    _outTree->Branch("aJet_SF_CSVMerr", aJets.SF_CSVMerr,
                     "SF_CSVMerr[naJets]/b");
    _outTree->Branch("aJet_SF_CSVTerr", aJets.SF_CSVTerr,
                     "SF_CSVTerr[naJets]/b");
    ...
    _outTree->Branch("deltaPullAngle2", &deltaPullAngle2,
                     "deltaPullAngle2/F");
    ...
    _outTree->Branch("weightTrigOrMu30", &weightTrigOrMu30,
                     "weightTrigOrMu30/F");
    _outTree->Branch("weightEleRecoAndId", &weightEleRecoAndId,
                     "weightEleRecoAndId/F");
    _outTree->Branch("weightEleTrigJetMETPart", &weightEleTrigJetMETPart,
                     "weightEleTrigJetMETPart/F");
    _outTree->Branch("weightEleTrigElePart", &weightEleTrigElePart,
                     "weightEleTrigElePart/F");
    _outTree->Branch("weightEleTrigEleAugPart", &weightEleTrigEleAugPart,
                     "weightEleTrigEleAugPart/F");


    _outTree->Branch("weightTrigMET80", &weightTrigMET80,
                     "weightTrigMET80/F");
    _outTree->Branch("weightTrigMET100", &weightTrigMET100,
                     "weightTrigMET100/F");
    _outTree->Branch("weightTrig2CJet20", &weightTrig2CJet20,
                     "weightTrig2CJet20/F");
    _outTree->Branch("weightTrigMET150", &weightTrigMET150,
                     "weightTrigMET150/F");
    _outTree->Branch("weightTrigMET802CJet", &weightTrigMET802CJet,
                     "weightTrigMET802CJet/F");
    _outTree->Branch("weightTrigMET1002CJet", &weightTrigMET1002CJet,
                     "weightTrigMET1002CJet/F");
    _outTree->Branch("weightTrigMETLP", &weightTrigMETLP,
                     "weightTrigMETLP/F");

    _outTree->Branch("weightTrig2012A", &weightTrig2012A,
                     "weightTrig2012A/F");
    _outTree->Branch("weightTrig2012ADiMuon", &weightTrig2012ADiMuon,
                     "weightTrig2012ADiMuon/F");
    _outTree->Branch("weightTrig2012ADiEle", &weightTrig2012ADiEle,
                     "weightTrig2012ADiEle/F");
    _outTree->Branch("weightTrig2012ASingleMuon", &weightTrig2012ASingleMuon,
                     "weightTrig2012ASingleMuon/F");
    _outTree->Branch("weightTrig2012ASingleEle", &weightTrig2012ASingleEle,
                     "weightTrig2012ASingleEle/F");
    _outTree->Branch("weightTrig2012AMuonPlusWCandPt",
                     &weightTrig2012AMuonPlusWCandPt,
                     "weightTrig2012AMuonPlusWCandPt/F");


    _outTree->Branch("weightTrig2012", &weightTrig2012, "weightTrig2012/F");
    _outTree->Branch("weightTrig2012DiMuon", &weightTrig2012DiMuon,
                     "weightTrig2012DiMuon/F");
    _outTree->Branch("weightTrig2012DiEle", &weightTrig2012DiEle,
                     "weightTrig2012DiEle/F");
    _outTree->Branch("weightTrig2012SingleMuon", &weightTrig2012SingleMuon,
                     "weightTrig2012SingleMuon/F");
    _outTree->Branch("weightTrig2012SingleEle", &weightTrig2012SingleEle,
                     "weightTrig2012SingleEle/F");
    _outTree->Branch("weightTrig2012MuonPlusWCandPt",
                     &weightTrig2012MuonPlusWCandPt,
                     "weightTrig2012MuonPlusWCandPt/F");


    _outTree->Branch("weightTrig2012AB", &weightTrig2012AB,
                     "weightTrig2012AB/F");
    _outTree->Branch("weightTrig2012ABDiMuon", &weightTrig2012ABDiMuon,
                     "weightTrig2012ABDiMuon/F");
    _outTree->Branch("weightTrig2012ABDiEle", &weightTrig2012ABDiEle,
                     "weightTrig2012ABDiEle/F");
    _outTree->Branch("weightTrig2012ABSingleMuon",
                     &weightTrig2012ABSingleMuon,
                     "weightTrig2012ABSingleMuon/F");
    _outTree->Branch("weightTrig2012ABSingleEle", &weightTrig2012ABSingleEle,
                     "weightTrig2012ABSingleEle/F");
    _outTree->Branch("weightTrig2012ABMuonPlusWCandPt",
                     &weightTrig2012ABMuonPlusWCandPt,
                     "weightTrig2012ABMuonPlusWCandPt/F");

    _outTree->Branch("weightTrig2012DiJet30MHT80",
                     &weightTrig2012DiJet30MHT80,
                     "weightTrig2012DiJet30MHT80/F");
    _outTree->Branch("weightTrig2012PFMET150", &weightTrig2012PFMET150,
                     "weightTrig2012PFMET150/F");
    _outTree->Branch("weightTrig2012SumpT100MET100",
                     &weightTrig2012SumpT100MET100,
                     "weightTrig2012SumpT100MET100/F");
    _outTree->Branch("weightTrig2012APFMET150orDiJetMET",
                     &weightTrig2012APFMET150orDiJetMET,
                     "weightTrig2012APFMET150orDiJetMET/F");
    _outTree->Branch("weightTrig2012BPFMET150orDiJetMET",
                     &weightTrig2012BPFMET150orDiJetMET,
                     "weightTrig2012BPFMET150orDiJetMET/F");
    _outTree->Branch("weightTrig2012CPFMET150orDiJetMET",
                     &weightTrig2012CPFMET150orDiJetMET,
                     "weightTrig2012CPFMET150orDiJetMET/F");




    _outTree->Branch("deltaPullAngleAK7", &deltaPullAngleAK7,
                     "deltaPullAngleAK7/F");
    _outTree->Branch("deltaPullAngle2AK7", &deltaPullAngle2AK7,
                     "deltaPullAngle2AK7/F");
    ...
    _outTree->Branch("vLepton_aodCombRelIso", vLeptons.aodCombRelIso,
                     "aodCombRelIso[nvlep]/F");
    _outTree->Branch("vLepton_pfCombRelIso", vLeptons.pfCombRelIso,
                     "pfCombRelIso[nvlep]/F");
    _outTree->Branch("vLepton_photonIso", vLeptons.photonIso,
                     "photonIso[nvlep]/F");
    _outTree->Branch("vLepton_neutralHadIso", vLeptons.neutralHadIso,
                     "neutralHadIso[nvlep]/F");
    _outTree->Branch("vLepton_chargedHadIso", vLeptons.chargedHadIso,
                     "chargedHadIso[nvlep]/F");
    _outTree->Branch("vLepton_chargedPUIso", vLeptons.chargedPUIso,
                     "chargedPUIso[nvlep]/F");
    _outTree->Branch("vLepton_particleIso", vLeptons.particleIso,
                     "particleIso[nvlep]/F");
    ...
    _outTree->Branch("vLepton_id80NoIso", vLeptons.id80NoIso,
                     "id80NoIso[nvlep]/F");
    ...
    _outTree->Branch("vLepton_pfCorrIso", vLeptons.pfCorrIso,
                     "pfCorrIso[nvlep]/F");
    _outTree->Branch("vLepton_id2012tight", vLeptons.id2012tight,
                     "id2012tight[nvlep]/F");
    _outTree->Branch("vLepton_idMVAnotrig", vLeptons.idMVAnotrig,
                     "idMVAnotrig[nvlep]/F");
    _outTree->Branch("vLepton_idMVAtrig", vLeptons.idMVAtrig,
                     "idMVAtrig[nvlep]/F");
    _outTree->Branch("vLepton_idMVApresel", vLeptons.idMVApresel,
                     "idMVApresel[nvlep]/F");
    _outTree->Branch("vLepton_innerHits", vLeptons.innerHits,
                     "innerHits[nvlep]/F");
    _outTree->Branch("vLepton_photonIsoDoubleCount",
                     vLeptons.photonIsoDoubleCount,
                     "photonIsoDoubleCount[nvlep]/F");
    ...
    _outTree->Branch("aLepton_aodCombRelIso", aLeptons.aodCombRelIso,
                     "aodCombRelIso[nalep]/F");
    _outTree->Branch("aLepton_pfCombRelIso", aLeptons.pfCombRelIso,
                     "pfCombRelIso[nalep]/F");
    _outTree->Branch("aLepton_photonIso", aLeptons.photonIso,
                     "photonIso[nalep]/F");
    _outTree->Branch("aLepton_neutralHadIso", aLeptons.neutralHadIso,
                     "neutralHadIso[nalep]/F");
    _outTree->Branch("aLepton_chargedHadIso", aLeptons.chargedHadIso,
                     "chargedHadIso[nalep]/F");
    _outTree->Branch("aLepton_chargedPUIso", aLeptons.chargedPUIso,
                     "chargedPUIso[nalep]/F");
    _outTree->Branch("aLepton_particleIso", aLeptons.particleIso,
                     "particleIso[nalep]/F");
    ...
    _outTree->Branch("aLepton_id80NoIso", aLeptons.id80NoIso,
                     "id80NoIso[nalep]/F");
    ...
    _outTree->Branch("aLepton_pfCorrIso", aLeptons.pfCorrIso,
                     "pfCorrIso[nalep]/F");
    _outTree->Branch("aLepton_id2012tight", aLeptons.id2012tight,
                     "id2012tight[nalep]/F");
    _outTree->Branch("aLepton_idMVAnotrig", aLeptons.idMVAnotrig,
                     "idMVAnotrig[nalep]/F");
    _outTree->Branch("aLepton_idMVAtrig", aLeptons.idMVAtrig,
                     "idMVAtrig[nalep]/F");
    _outTree->Branch("aLepton_idMVApresel", aLeptons.idMVApresel,
                     "idMVApresel[nalep]/F");
    _outTree->Branch("aLepton_innerHits", aLeptons.innerHits,
                     "innerHits[nalep]/F");
    _outTree->Branch("aLepton_photonIsoDoubleCount",
                     aLeptons.photonIsoDoubleCount,
                     "photonIsoDoubleCount[nalep]/F");
    ...
    _outTree->Branch("Sv_dist3D_norm", &IVF.dist3D_norm,
                     "dist3D_norm[nSvs]/F");
    //IVF higgs candidate
...
//   //SimBHadron
    ...
    _outTree->Branch("SimBsH", &SimBsH,
                     "mass/F:pt/F:eta:phi/F:dR/F:dPhi/F:dEta/F");
    ...
    _outTree->Branch("METtype1p2corr", &METtype1p2corr,
                     "et/F:sumet:sig/F:phi/F");
    _outTree->Branch("METnoPUtype1corr", &METnoPUtype1corr,
                     "et/F:sumet:sig/F:phi/F");
    _outTree->Branch("METnoPUtype1p2corr", &METnoPUtype1p2corr,
                     "et/F:sumet:sig/F:phi/F");
    ...
    _outTree->Branch("minDeltaPhijetMET", &minDeltaPhijetMET,
                     "minDeltaPhijetMET/F");
    _outTree->Branch("jetPt_minDeltaPhijetMET", &jetPt_minDeltaPhijetMET,
                     "jetPt_minDeltaPhijetMET/F");
    ...
    _outTree->Branch("totalKinematics", &totalKinematics,
                     "totalKinematics/b");
    ...
    _outTree->Branch("trackingfailureFlag", &trackingfailureFlag,
                     "trackingfailureFlag/b");
    ...
    for (unsigned int iFile = 0; iFile < inputFiles_.size(); ++iFile) {
        ...
        if (inFile == 0) {
            std::cout << "FAILED " << inputFiles_[iFile] << std::endl;
            continue;
        }
        // loop the events
        ...
        for (ev.toBegin(); !ev.atEnd(); ++ev, ++ievt) {
            if (ievt <= skipEvents_)
                continue;
            if (maxEvents_ >= 0) {
                if (ievt > maxEvents_ + skipEvents_)
                    break;
            }
            ...
            if (EVENT.run < runMin_ && runMin_ > 0)
                continue;
            if (EVENT.run > runMax_ && runMax_ > 0)
                continue;
...
*/
            ...
            if (isMC_) {
                ...
                // PU weights // Run2011A
                std::map < int, unsigned int >::const_iterator puit =
                    aux.puInfo.pus.find(0);
                int npu = puit->second;
                ...
                // PU weight Run2011B
                // PU weight Run2011B
                std::map < int, unsigned int >::const_iterator puit0 =
                    aux.puInfo.pus.find(0);
                std::map < int, unsigned int >::const_iterator puitm1 =
                    aux.puInfo.pus.find(-1);
                std::map < int, unsigned int >::const_iterator puitp1 =
                    aux.puInfo.pus.find(+1);
                PU0 = puit0->second;
                PUp1 = puitp1->second;
                PUm1 = puitm1->second;
...
//        PUweight2011B = lumiWeights2011B.weight3D( puitm1->second, puit0->second,puitp1->second); 
            ...
            if (!((evt.getBranchNameFor(ev, "source")).empty())) {
                ...
                for (unsigned int i = 0; i < pup_.size(); ++i) {
                    ...
                    if (id == 11) {
                        l.SetPxPyPzE(hepeup_.PUP[i][0], hepeup_.PUP[i][1],
                                     hepeup_.PUP[i][2], hepeup_.PUP[i][3]);
                        lCheck = true;
                    }
                    if (id == -11) {
                        lbar.SetPxPyPzE(hepeup_.PUP[i][0], hepeup_.PUP[i][1],
                                        hepeup_.PUP[i][2], hepeup_.PUP[i][3]);
                        lbarCheck = true;
                    }
                    if (id == 12) {
                        vl.SetPxPyPzE(hepeup_.PUP[i][0], hepeup_.PUP[i][1],
                                      hepeup_.PUP[i][2], hepeup_.PUP[i][3]);
                        vlCheck = true;
                    }
                    if (id == -12) {
                        vlbar.SetPxPyPzE(hepeup_.PUP[i][0], hepeup_.PUP[i][1],
                                         hepeup_.PUP[i][2],
                                         hepeup_.PUP[i][3]);
                        vlbarCheck = true;
                    }
                ...
                if (vlCheck && vlbarCheck)
                    V_tlv = vl + vlbar; // ZtoNuNu
                if (lCheck && vlbarCheck)
                    V_tlv = l + vlbar;  // WToLNu
                if (lbarCheck && vlCheck)
                    V_tlv = lbar + vl;  // WToLNu       
                lheV_pt = V_tlv.Pt();
            }
            ...
            if (fromCandidate) {
                fwlite::Handle < std::vector < VHbbCandidate >
                    >vhbbCandHandleZ;
                ...
                fwlite::Handle < std::vector < VHbbCandidate >
                    >vhbbCandHandle;
                vhbbCandHandle.getByLabel(ev,
                                          "hbbHighestPtHiggsPt30Candidates");
                ...
                if (isMC_) {
                    ...
                    for (size_t j = 0; j < modifiedEvent.simpleJets2.size();
                         j++) {
...
//               std::cout << "Original " << orig.p4.Pt() << " == " << origRemade.p4.Pt() << " using CHS2011 " << corr2011.p4.Pt() << " final: " << modifiedEvent.simpleJets2[j].p4.Pt() << std::endl;
                        ...
                        TLorentzVector & mcp4 =
                            modifiedEvent.simpleJets2[j].bestMCp4;
                        if ((fabs(p4.Pt() - mcp4.Pt()) / p4.Pt()) < 0.5) {      //Limit the effect to the core 
                            float cor =
                                (p4.Pt() +
                                 resolutionBias(fabs(p4.Eta())) * (p4.Pt() -
                                                                   mcp4.
                                                                   Pt())) /
                                p4.Pt();
                            p4.SetPtEtaPhiE(p4.Pt() * cor, p4.Eta(), p4.Phi(),
                                            p4.E() * cor);
                        }
                    }
                } else {
            ...
            if (cand->size() > 1) {
                std::cout << "MULTIPLE CANDIDATES: " << cand->
                    size() << std::endl;
            }
            if (cand->at(0).candidateType == VHbbCandidate::Wmun
                || cand->at(0).candidateType == VHbbCandidate::Wen) {
                cand = candW;
                isW = true;
            }
            if (cand->size() == 0) {
...
//        if(vhCand.FatH.FatHiggsFlag)  vhCand.FatH.subjetsSize; 
                ...
                if (nfathFilterJets == 2) {
                    FatH.filteredmass =
                        (vhCand.FatH.jets[0].p4 + vhCand.FatH.jets[1].p4).M();
                    FatH.filteredpt =
                        (vhCand.FatH.jets[0].p4 +
                         vhCand.FatH.jets[1].p4).Pt();
                    FatH.filteredeta =
                        (vhCand.FatH.jets[0].p4 +
                         vhCand.FatH.jets[1].p4).Eta();
                    FatH.filteredphi =
                        (vhCand.FatH.jets[0].p4 +
                         vhCand.FatH.jets[1].p4).Phi();
                    ...
                    FatH.filteredmass =
                        (vhCand.FatH.jets[0].p4 + vhCand.FatH.jets[1].p4 +
                         vhCand.FatH.jets[2].p4).M();
                    FatH.filteredpt =
                        (vhCand.FatH.jets[0].p4 + vhCand.FatH.jets[1].p4 +
                         vhCand.FatH.jets[2].p4).Pt();
                    FatH.filteredeta =
                        (vhCand.FatH.jets[0].p4 + vhCand.FatH.jets[1].p4 +
                         vhCand.FatH.jets[2].p4).Eta();
                    FatH.filteredphi =
                        (vhCand.FatH.jets[0].p4 + vhCand.FatH.jets[1].p4 +
                         vhCand.FatH.jets[2].p4).Phi();
                    fathFilterJets.cosTheta[0] = vhCand.FatH.helicities[0];
                    fathFilterJets.cosTheta[1] = vhCand.FatH.helicities[1];
            ...
            if (vhCand.H.HiggsFlag) {
                ...
                H.dR =
                    deltaR(vhCand.H.jets[0].p4.Eta(),
                           vhCand.H.jets[0].p4.Phi(),
                           vhCand.H.jets[1].p4.Eta(),
                           vhCand.H.jets[1].p4.Phi());
                H.dPhi =
                    deltaPhi(vhCand.H.jets[0].p4.Phi(),
                             vhCand.H.jets[1].p4.Phi());
                H.dEta =
                    TMath::Abs(vhCand.H.jets[0].p4.Eta() -
                               vhCand.H.jets[1].p4.Eta());
                HVdPhi = fabs(deltaPhi(vhCand.H.p4.Phi(), vhCand.V.p4.Phi()));
                HVMass = (vhCand.H.p4 + vhCand.V.p4).M();
                HMETdPhi =
                    fabs(deltaPhi
                         (vhCand.H.p4.Phi(), vhCand.V.mets.at(0).p4.Phi()));
//eltaPullAngle = vhCand.H.deltaTheta;

                deltaPullAngle =
                    VHbbCandidateTools::getDeltaTheta(vhCand.H.jets[0],
                                                      vhCand.H.jets[1]);
                deltaPullAngle2 =
                    VHbbCandidateTools::getDeltaTheta(vhCand.H.jets[1],
                                                      vhCand.H.jets[0]);
...
//              mu2.SetMag( mu2.Mag() - vhCand.V.muons[1].emEnergy - vhCand.V.muons[1].hadEnergy);
                ...
                fakeMET.phi = sum.Phi();
                fakeMET.sumet =
                    vhCand.V.mets.at(0).sumEt - mu1.Pt() - mu2.Pt();
            }
            ...
            // track sharing flags:
            ////////
            TkSharing.HiggsCSVtkSharing = TkSharing.HiggsIPtkSharing =
                TkSharing.HiggsSVtkSharing = TkSharing.FatHiggsCSVtkSharing =
                TkSharing.FatHiggsIPtkSharing =
                TkSharing.FatHiggsSVtkSharing = false;

            // csv tracks
            if (vhCand.H.HiggsFlag) {

                if (vhCand.H.jets[0].csvNTracks > 0
                    && vhCand.H.jets[1].csvNTracks > 0) {
                    for (int t = 0; t != vhCand.H.jets[0].csvNTracks; t++) {
                        for (int ti = 0; ti != vhCand.H.jets[1].csvNTracks;
                             ti++) {
                            if ((int) vhCand.H.jets[0].csvTrackIds[t] ==
                                (int) vhCand.H.jets[1].csvTrackIds[ti]) {
                ...
                // ip tracks
                if (vhCand.H.jets[0].btagNTracks > 0
                    && vhCand.H.jets[1].btagNTracks > 0) {
                    for (int t = 0; t != vhCand.H.jets[0].btagNTracks; t++) {
                        for (int ti = 0; ti != vhCand.H.jets[1].btagNTracks;
                             ti++) {
                            if ((int) vhCand.H.jets[0].btagTrackIds[t] ==
                                (int) vhCand.H.jets[1].btagTrackIds[ti]) {
                ...
                // sv tracks
                if (vhCand.H.jets[0].vtxNTracks > 0
                    && vhCand.H.jets[1].vtxNTracks > 0) {
                    for (int t = 0; t != vhCand.H.jets[0].vtxNTracks; t++) {
                        for (int ti = 0; ti != vhCand.H.jets[1].vtxNTracks;
                             ti++) {
                            if ((int) vhCand.H.jets[0].vtxTrackIds[t] ==
                                (int) vhCand.H.jets[1].vtxTrackIds[ti]) {
            ...
            if (vhCand.FatH.FatHiggsFlag && nfathFilterJets > 1) {
                ...
                // csv tracks
                if (vhCand.FatH.jets[0].csvNTracks > 0
                    && vhCand.FatH.jets[1].csvNTracks > 0) {
                    for (int t = 0; t != vhCand.FatH.jets[0].csvNTracks; t++) {
                        for (int ti = 0; ti != vhCand.FatH.jets[1].csvNTracks;
                             ti++) {
                            if ((int) vhCand.FatH.jets[0].csvTrackIds[t] ==
                                (int) vhCand.FatH.jets[1].csvTrackIds[ti]) {
                ...
                // ip tracks
                if (vhCand.FatH.jets[0].btagNTracks > 0
                    && vhCand.FatH.jets[1].btagNTracks > 0) {
                    for (int t = 0; t != vhCand.FatH.jets[0].btagNTracks; t++) {
                        for (int ti = 0;
                             ti != vhCand.FatH.jets[1].btagNTracks; ti++) {
                            if ((int) vhCand.FatH.jets[0].btagTrackIds[t] ==
                                (int) vhCand.FatH.jets[1].btagTrackIds[ti]) {
                ...
                // sv tracks
                if (vhCand.FatH.jets[0].vtxNTracks > 0
                    && vhCand.FatH.jets[1].vtxNTracks > 0) {
                    for (int t = 0; t != vhCand.FatH.jets[0].vtxNTracks; t++) {
                        for (int ti = 0; ti != vhCand.FatH.jets[1].vtxNTracks;
                             ti++) {
                            if ((int) vhCand.FatH.jets[0].vtxTrackIds[t] ==
                                (int) vhCand.FatH.jets[1].vtxTrackIds[ti]) {
...
//        REAL ERROR MATRIX
            ...
            for (int j = 0; j < nSvs && j < MAXB; ++j) {
                const GlobalVector flightDir =
                    flightDirection(recoPv, svc[j]);
                reco::SecondaryVertex recoSv(recoVtxPv, svc[j], flightDir,
                                             true);
                IVF.set(recoSv, recoPv, j);
            }
            if (nSvs > 1) {
                TLorentzVector BCands_H1, BCands_H2, BCands_H;
                BCands_H1.SetPtEtaPhiM(IVF.pt[0], IVF.eta[0], IVF.phi[0],
                                       IVF.massBcand[0]);
                BCands_H2.SetPtEtaPhiM(IVF.pt[1], IVF.eta[1], IVF.phi[1],
                                       IVF.massBcand[1]);
                BCands_H = BCands_H1 + BCands_H2;
                SVH.dR =
                    deltaR(IVF.eta[0], IVF.phi[0], IVF.eta[1], IVF.phi[1]);
            ...
            if (isMC_) {
                ...
                if (nSimBs > 1) {
                    ...
                    SimBs_H1.SetPtEtaPhiM(SimBs.pt[0], SimBs.eta[0],
                                          SimBs.phi[0], SimBs.mass[0]);
                    SimBs_H2.SetPtEtaPhiM(SimBs.pt[1], SimBs.eta[1],
                                          SimBs.phi[1], SimBs.mass[1]);
                    SimBs_H = SimBs_H1 + SimBs_H2;
                    SimBsH.dR =
                        deltaR(SimBs.eta[0], SimBs.phi[0], SimBs.eta[1],
                               SimBs.phi[1]);
            ...
            if (fromCandidate) {
                ...
                for (unsigned int j = 0; j < vhCand.H.jets.size(); j++) {
                    ...
                    if (fabs
                        (deltaPhi
                         (vhCand.V.mets.at(0).p4.Phi(),
                          vhCand.H.jets[j].p4.Phi())) < minDeltaPhijetMET) {
                        minDeltaPhijetMET =
                            fabs(deltaPhi
                                 (vhCand.V.mets.at(0).p4.Phi(),
                                  vhCand.H.jets[j].p4.Phi()));
                ...
                for (unsigned int j = 0; j < vhCand.additionalJets.size();
                     j++) {
...
/*	      if (fabs(deltaPhi( vhCand.V.mets.at(0).p4.Phi(), vhCand.additionalJets[j].p4.Phi())) < minDeltaPhijetMET) 
		{
                  ...
                  jetPt_minDeltaPhijetMET=vhCand.additionalJets[j].p4.Pt();
                    ...
                    if ((isW && !useHighestPtHiggsW) || (!isW && !useHighestPtHiggsZ))  // btag SF computed using only H-jets if best-H made with dijetPt rather than best CSV
                    {
                        if (vhCand.additionalJets[j].p4.Pt() > 20)
                            btagJetInfos.push_back(btagEff.
                                                   jetInfo(vhCand.
                                                           additionalJets
                                                           [j]));
                    }
                ...
                for (unsigned int j = 0; j < iEvent->simpleJets2.size(); j++) {
                    ...
                    if (iEvent->simpleJets2[j].p4.Pt() > 20
                        && fabs(iEvent->simpleJets2[j].p4.Eta()) < 2.5
                        &&
                        fabs(deltaPhi
                             (vhCand.V.mets.at(0).p4.Phi(),
                              iEvent->simpleJets2[j].p4.Phi())) <
                        minDeltaPhijetMET) {
                        minDeltaPhijetMET =
                            fabs(deltaPhi
                                 (vhCand.V.mets.at(0).p4.Phi(),
                                  iEvent->simpleJets2[j].p4.Phi()));
                        jetPt_minDeltaPhijetMET =
                            iEvent->simpleJets2[j].p4.Pt();
                    }
                    ...
                    if ((isW && !useHighestPtHiggsW)
                        || (!isW && !useHighestPtHiggsZ)) {
                        if (iEvent->simpleJets2[j].p4.Pt() > 20
                            && fabs(iEvent->simpleJets2[j].p4.Eta()) < 2.5)
                            btagJetInfos.push_back(btagEff.
                                                   jetInfo(iEvent->
                ...
                if (vhCand.H.HiggsFlag) {
                    for (unsigned int j = 0; j < vhCand.H.jets.size(); j++) {
                        ...
                        if (!
                            ((isW && !useHighestPtHiggsW)
                             || (!isW && !useHighestPtHiggsZ))) {
                            btagJetInfos.push_back(btagEff.
                                                   jetInfo(vhCand.H.jets[j]));
                        }
                    }
                }               // HiggsFlag
            ...
            if (Vtype == VHbbCandidate::Zmumu) {
                ...
                float cweightID =
                    triggerWeight.scaleMuID(vLeptons.pt[0],
                                            vLeptons.eta[0]) *
                    triggerWeight.scaleMuID(vLeptons.pt[1], vLeptons.eta[1]);
                float weightTrig1 =
                    triggerWeight.scaleMuIsoHLT(vLeptons.pt[0],
                                                vLeptons.eta[0]);
                float weightTrig2 =
                    triggerWeight.scaleMuIsoHLT(vLeptons.pt[1],
                                                vLeptons.eta[1]);
                float cweightTrig =
                    weightTrig1 + weightTrig2 - weightTrig1 * weightTrig2;
                //2011
                weightTrig = cweightID * cweightTrig;


                weightTrig2012DiMuon =
                    triggerWeight.doubleMuon2012A(vLeptons.pt[0],
                                                  vLeptons.eta[0],
                                                  vLeptons.pt[1],
                                                  vLeptons.eta[1]);
                float weightTrig2012SingleMuonMu1 =
                    triggerWeight.singleMuon2012A(vLeptons.pt[0],
                                                  vLeptons.eta[0]);
                float weightTrig2012SingleMuonMu2 =
                    triggerWeight.singleMuon2012A(vLeptons.pt[1],
                                                  vLeptons.eta[1]);
                weightTrig2012SingleMuon =
                    weightTrig2012SingleMuonMu1 +
                    weightTrig2012SingleMuonMu2 -
                    weightTrig2012SingleMuonMu1 * weightTrig2012SingleMuonMu2;
                weightTrig2012 =
                    weightTrig2012SingleMuon *
                    triggerWeight.muId2012A(vLeptons.pt[0],
                                            vLeptons.eta[0]) *
                    triggerWeight.muId2012A(vLeptons.pt[1], vLeptons.eta[1]);

                weightTrig2012ABDiMuon =
                    triggerWeightAB.doubleMuon2012A(vLeptons.pt[0],
                                                    vLeptons.eta[0],
                                                    vLeptons.pt[1],
                                                    vLeptons.eta[1]);
                float weightTrig2012ABSingleMuonMu1 =
                    triggerWeightAB.singleMuon2012A(vLeptons.pt[0],
                                                    vLeptons.eta[0]);
                float weightTrig2012ABSingleMuonMu2 =
                    triggerWeightAB.singleMuon2012A(vLeptons.pt[1],
                                                    vLeptons.eta[1]);
                weightTrig2012ABSingleMuon =
                    weightTrig2012ABSingleMuonMu1 +
                    weightTrig2012ABSingleMuonMu2 -
                    weightTrig2012ABSingleMuonMu1 *
                    weightTrig2012ABSingleMuonMu2;
                weightTrig2012AB =
                    weightTrig2012ABSingleMuon *
                    triggerWeightAB.muId2012A(vLeptons.pt[0],
                                              vLeptons.eta[0]) *
                    triggerWeightAB.muId2012A(vLeptons.pt[1],
                                              vLeptons.eta[1]);
            ...
            if (Vtype == VHbbCandidate::Zee) {
                ...
                weightEleRecoAndId =
                    triggerWeight.scaleID95Ele(vLeptons.pt[0],
                                               vLeptons.eta[0]) *
                    triggerWeight.scaleRecoEle(vLeptons.pt[0],
                                               vLeptons.eta[0]) *
                    triggerWeight.scaleID95Ele(vLeptons.pt[1],
                                               vLeptons.eta[1]) *
                    triggerWeight.scaleRecoEle(vLeptons.pt[1],
                                               vLeptons.eta[1]);
                weightEleTrigElePart =
                    triggerWeight.scaleDoubleEle17Ele8(pt, eta);
                weightEleTrigEleAugPart =
                    triggerWeight.scaleDoubleEle17Ele8Aug(pt, eta);
                weightTrig =
                    (weightEleTrigElePart * 1.14 +
                     weightEleTrigEleAugPart * 0.98) / 2.12 *
                    weightEleRecoAndId;

                weightTrig2012ADiEle =
                    triggerWeight.doubleEle2012A(vLeptons.pt[0],
                                                 vLeptons.eta[0],
                                                 vLeptons.pt[1],
                                                 vLeptons.eta[1]);
                float weightTrig2012ASingleEle1 =
                    triggerWeight.singleEle2012Awp95(vLeptons.pt[0],
                                                     vLeptons.eta[0]);
                float weightTrig2012ASingleEle2 =
                    triggerWeight.singleEle2012Awp95(vLeptons.pt[1],
                                                     vLeptons.eta[1]);
                weightTrig2012ASingleEle =
                    weightTrig2012ASingleEle1 + weightTrig2012ASingleEle2 -
                    weightTrig2012ASingleEle1 * weightTrig2012ASingleEle2;
                weightTrig2012A =
                    weightTrig2012ADiEle *
                    triggerWeight.eleId2012A(vLeptons.pt[0],
                                             vLeptons.eta[0]) *
                    triggerWeight.eleId2012A(vLeptons.pt[1], vLeptons.eta[1]);
                ...
                weightTrig2012DiEle =
                    triggerWeight.doubleEle2012A(vLeptons.pt[0],
                                                 ...
                                                 vLeptons.pt[1],
                                                 vLeptons.eta[1]);
                float weightTrig2012SingleEle1 =
                    triggerWeight.singleEle2012Awp95(vLeptons.pt[0],
                                                     vLeptons.eta[0]);
                float weightTrig2012SingleEle2 =
                    triggerWeight.singleEle2012Awp95(vLeptons.pt[1],
                                                     vLeptons.eta[1]);
                weightTrig2012SingleEle =
                    weightTrig2012SingleEle1 + weightTrig2012SingleEle2 -
                    weightTrig2012SingleEle1 * weightTrig2012SingleEle2;
                weightTrig2012 =
                    weightTrig2012DiEle *
                    triggerWeight.eleId2012A(vLeptons.pt[0],
                ...
                weightTrig2012ABDiEle =
                    triggerWeightAB.doubleEle2012A(vLeptons.pt[0],
                                                   vLeptons.eta[0],
                                                   vLeptons.pt[1],
                                                   vLeptons.eta[1]);
                float weightTrig2012ABSingleEle1 =
                    triggerWeightAB.singleEle2012Awp95(vLeptons.pt[0],
                                                       vLeptons.eta[0]);
                float weightTrig2012ABSingleEle2 =
                    triggerWeightAB.singleEle2012Awp95(vLeptons.pt[1],
                                                       vLeptons.eta[1]);
                weightTrig2012ABSingleEle =
                    weightTrig2012ABSingleEle1 + weightTrig2012ABSingleEle2 -
                    weightTrig2012ABSingleEle1 * weightTrig2012ABSingleEle2;
                weightTrig2012AB =
                    weightTrig2012ABDiEle *
                    triggerWeightAB.eleId2012A(vLeptons.pt[0],
                                               vLeptons.eta[0]) *
                    triggerWeightAB.eleId2012A(vLeptons.pt[1],
                                               vLeptons.eta[1]);
            }
            if (Vtype == VHbbCandidate::Wmun) {
                ...
                vLeptons.set(vhCand.V.muons[0], 0, 13, aux);
                float cweightID =
                    triggerWeight.scaleMuID(vLeptons.pt[0], vLeptons.eta[0]);
                float weightTrig1 =
                    triggerWeight.scaleMuIsoHLT(vLeptons.pt[0],
                                                vLeptons.eta[0]);
                float cweightTrig = weightTrig1;
                weightTrig = cweightID * cweightTrig;
                float weightTrig1OrMu30 =
                    triggerWeight.scaleMuOr30IsoHLT(vLeptons.pt[0],
                                                    vLeptons.eta[0]);
                weightTrigOrMu30 = cweightID * weightTrig1OrMu30;
                weightTrig2012ASingleMuon =
                    triggerWeight.singleMuon2012A(vLeptons.pt[0],
                                                  vLeptons.eta[0]);
                weightTrig2012AMuonPlusWCandPt =
                    weightTrig2012ASingleMuon +
                    triggerWeight.muPlusWCandPt2012A_legMu(vLeptons.pt[0],
                                                           vLeptons.eta[0]) *
                    triggerWeight.muPlusWCandPt2012A_legW(vhCand.V.p4.Pt(),
                                                          0);
                weightTrig2012A =
                    weightTrig2012ASingleMuon *
                    triggerWeight.muId2012A(vLeptons.pt[0], vLeptons.eta[0]);
                ...
                weightTrig2012ABMuonPlusWCandPt =
                    weightTrig2012ABSingleMuon +
                    triggerWeightAB.muPlusWCandPt2012A_legMu(vLeptons.pt[0],
                                                             vLeptons.
                                                             eta[0]) *
                    triggerWeightAB.muPlusWCandPt2012A_legW(vhCand.V.p4.Pt(),
                                                            0);
                weightTrig2012AB =
                    weightTrig2012ABSingleMuon *
                    triggerWeightAB.muId2012A(vLeptons.pt[0],
            ...
            if (Vtype == VHbbCandidate::Wen) {
                ...
                firstAddEle = 1;
                weightTrigMay =
                    triggerWeight.scaleSingleEleMay(vLeptons.pt[0],
                                                    vLeptons.eta[0]);
                weightTrigV4 =
                    triggerWeight.scaleSingleEleV4(vLeptons.pt[0],
                                                   vLeptons.eta[0]);
                weightEleRecoAndId =
                    triggerWeight.scaleID80Ele(vLeptons.pt[0],
                                               vLeptons.eta[0]) *
                    triggerWeight.scaleRecoEle(vLeptons.pt[0],
                                               vLeptons.eta[0]);
                weightEleTrigJetMETPart =
                    triggerWeight.scaleJet30Jet25(jet30pt,
                                                  jet30eta) *
                    triggerWeight.scalePFMHTEle(MET.et);
...
//        weightTrig = weightTrigMay * 0.187 + weightTrigV4 * (1.-0.187); //FIXME: use proper lumi if we reload 2.fb
                ...
                weightTrig2012ASingleEle =
                    triggerWeight.singleEle2012Awp80(vLeptons.pt[0],
                                                     vLeptons.eta[0]);
                weightTrig2012A =
                    weightTrig2012ASingleEle *
                    triggerWeight.eleId2012Awp80(vLeptons.pt[0],
                                                 vLeptons.eta[0]);
                ...
                weightTrig2012ABSingleEle =
                    triggerWeightAB.singleEle2012Awp80(vLeptons.pt[0],
                                                       vLeptons.eta[0]);
                weightTrig2012AB =
                    weightTrig2012ABSingleEle *
                    triggerWeightAB.eleId2012Awp80(vLeptons.pt[0],
                                                   vLeptons.eta[0]);
            ...
            if (isMC_) {
                ...
                weightTrig2CJet20 =
                    triggerWeight.scale2CentralJet(jet10pt, jet10eta);
                ...
                weightTrig2012DiJet30MHT80 =
                    triggerWeight.scaleDiJet30MHT80_2012A(vhCand.V.mets.at(0).
                                                          p4.Pt());
                ...
                weightTrig2012APFMET150orDiJetMET =
                    triggerWeight.scalePFMET150orDiJetMET_2012A(vhCand.V.mets.
                                                                at(0).p4.
                                                                Pt());
                weightTrig2012BPFMET150orDiJetMET =
                    triggerWeight.scalePFMET150orDiJetMET_2012B(vhCand.V.mets.
                                                                at(0).p4.
                                                                Pt());
                weightTrig2012CPFMET150orDiJetMET =
                    triggerWeight.scalePFMET150orDiJetMET_2012C(vhCand.V.mets.
                                                                at(0).p4.
                                                                Pt());
            ...
            if (Vtype == VHbbCandidate::Znn) {
                nvlep = 0;
                float weightTrig1 =
                    triggerWeight.scaleMetHLT(vhCand.V.mets.at(0).p4.Pt());
                weightTrigMETLP = weightTrig1;
                weightTrig =
                    weightTrigMET150 + weightTrigMET802CJet -
                    weightTrigMET802CJet * weightTrigMET150;
...
//          weightTrig2012A = 
            ...
            if (fromCandidate) {
                ...
                for (size_t j = firstAddEle; j < vhCand.V.electrons.size();
                     j++)
                    aLeptons.set(vhCand.V.electrons[j], nalep++, 11, aux);
            } else {
                for (size_t j = 0; j < iEvent->muInfo.size(); j++) {
                    if ((j != vhCand.V.firstLepton
                         && j != vhCand.V.secondLepton)
                        || ((Vtype != VHbbCandidate::Wmun)
                            && (Vtype != VHbbCandidate::Zmumu)))
                        aLeptons.set(iEvent->muInfo[j], nalep++, 13, aux);
                }
                for (size_t j = 0; j < iEvent->eleInfo.size(); j++) {
                    if ((j != vhCand.V.firstLepton
                         && j != vhCand.V.secondLepton)
                        || ((Vtype != VHbbCandidate::Wen)
                            && (Vtype != VHbbCandidate::Zee)))
            ...
            if (isMC_) {
                ...
                if (btagJetInfos.size() < 10) {
                    btag1T2CSF =
                        btag.weight < BTag1Tight2CustomFilter >
                        (btagJetInfos);
                    btag2TSF =
                        btag.weight < BTag2TightFilter > (btagJetInfos);
                    btag1TSF =
                        btag.weight < BTag1TightFilter > (btagJetInfos);
                    btagA0CSF =
                        btag.weight < BTagAntiMax0CustomFilter >
                        (btagJetInfos);
                    btagA0TSF =
                        btag.weight < BTagAntiMax0TightFilter >
                        (btagJetInfos);
                    btag2CSF =
                        btag.weight < BTag2CustomFilter > (btagJetInfos);
                    btag1TA1C =
                        btag.weight < BTag1TightAndMax1CustomFilter >
                        (btagJetInfos);
                } else {
                    std::
                        cout << "WARNING:  combinatorics for " <<
                        btagJetInfos.
                        size() << " jets is too high (>=10). use SF=1 " <<
                        std::endl;
            ...
            if (maxBtag > -99999) {
                TopHypo topQuark =
                    TopMassReco::topMass(leptonForTop, bJet,
                                         vhCand.V.mets.at(0).p4);
...
//   genZstar.eta = aux.mcZ[i].p4.Eta();
                ...
                if (aux.mcZ[i].dauid.size() > 1) {
                    if (abs(aux.mcZ[i].dauid[0]) == 13
                        || abs(aux.mcZ[i].dauid[0]) == 11) {
...
//            genZ.eta = aux.mcZ[i].p4.Eta();
            ...
            for (unsigned int i = 0; i < aux.mcW.size(); i++) {
                ...
                if (aux.mcW[i].dauid.size() > 1
                    && (abs(aux.mcW[i].dauid[0]) == 13
                        || abs(aux.mcW[i].dauid[0]) == 11)) {
            ...
            if (vhCand.H.HiggsFlag) {
                if (!fromCandidate) {
                    std::vector < VHbbEvent::SimpleJet >
                        ak7wrt1(iEvent->simpleJets3);
                    std::vector < VHbbEvent::SimpleJet >
                        ak7wrt2(iEvent->simpleJets3);
                    if (ak7wrt1.size() > 1) {
                        CompareDeltaR deltaRComparatorJ1(vhCand.H.jets[0].p4);
                        CompareDeltaR deltaRComparatorJ2(vhCand.H.jets[1].p4);
                        std::sort(ak7wrt1.begin(), ak7wrt1.end(),
                                  deltaRComparatorJ1);
                        std::sort(ak7wrt2.begin(), ak7wrt2.end(),
                                  deltaRComparatorJ2);
                        ...
                        std::sort(ak7_matched.begin(), ak7_matched.end(),
                                  ptComparator);
                        if (ak7_matched[0].p4.DeltaR(vhCand.H.jets[0].p4) <
                            0.5 and ak7_matched[1].p4.DeltaR(vhCand.H.jets[1].
                                                             p4) < 0.5) {
                            deltaPullAngleAK7 =
                                VHbbCandidateTools::
                                getDeltaTheta(ak7_matched[0], ak7_matched[1]);
                            deltaPullAngle2AK7 =
                                VHbbCandidateTools::
                                getDeltaTheta(ak7_matched[1], ak7_matched[0]);
                        }
                    }
                }
            }                   //HiggsFlag
